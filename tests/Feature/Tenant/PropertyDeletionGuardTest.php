...
class PropertyDeletionGuardTest extends TestCase
{
    ...
    //   入力チェックが無くても所属チェックが止める（Bug #48 の型）。property_id のエラーを名指しで見るアサートは
    //   それでも落ちる（2026-09-14 に変異で実測）が、「何も書かれない」のアサートは所属チェックで満たされてしまう。
    //   そこで歯止めを通さずに物件を直接 delete() して、その下に生きている区画を残した状態を作り、
    //   入力チェックを外すとレコードが実際に作られる（＝どちらのアサートも効く）形にする。
    // ============================================================
