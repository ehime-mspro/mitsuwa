 ...
 *   ⚠ 逆向きの誤りもある——根のチェーンにクロージャを渡す形
 *   （`ReContract::where(function ($q) { ...; $other->update([...]); })->get();`）だと、
 *   クロージャ内の**無関係な** ->update( も同じ 1 文に入るので**誤検出**する。
 *   2026-08-04 実測では該当形は `DeletionBlockers::forProject()` の 1 箇所だけで、
 *   そこに無関係な update は無いので現状は無害。誤検出は必ずテストが落ちて気づけるので
 *   （無音で漏れる見落としと違い）この方向の誤りは許容している。
 *
