...
class MansionImportTest extends TestCase
{
    ...
    use ParsesForms;
    // プレビュー → 確定の往復（2026-09-14 に切り出した。テナントの区画の取込と共用）
     ...
     *   （Bug #43 / #46 / #49）。
     *
     * ⚠ **未カバー**: 「二重契約の**文言**」そのものは固定していない。①は
     *   `warnings[0]['message']` の部分一致、②はその値を画面と突き合わせる形なので、
     *   コントローラ側で文言を書き換えても、部分一致の
     *   「には既に契約中の入居者がいます」さえ残っていれば赤にならない。
     *   ここで守りたいのは文面でなく**チャネル**なので意図的にそうしてある。
     */
