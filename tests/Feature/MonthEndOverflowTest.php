 ...
 *   今日を月の途中（3/15 など）や、前月に同じ日がある月末（8/31 の前月 = 7/31 など）にすると、
 *   正しい順序と誤った順序が同じ値になり、**修正を元に戻してもこのテストは緑のまま**になる
...
class MonthEndOverflowTest extends TestCase
{
     ...
     *
     * ⚠ 溢れない日（3/15・8/31 など）に差し替えると、正しい順序と誤った順序が同じ値になり、
     *   修正を元に戻してもこのファイルは緑のままになる。日付を差し替えた瞬間にここで落として気づけるようにする
