...
trait ParsesSortLinks
{
     ...
     *
     * ⚠ 探索は `<th>` に閉じない（クリアリンクなどページ全体が対象）。span 分岐は
     *   その探索範囲をわずかに広げる点に留意すること。
     */
     ...
     *
     * ⚠ 複数の `<th>` が同じラベルに一致する場合は**最初の 1 つ**を返す（意図的）。
     *   `array_reverse($cells)` のような変異もテストでは検出できないが、同一ラベルの
     *   `<th>` が並ぶビューは現状無いため、fixture 専用のテストは置かない。
     *
