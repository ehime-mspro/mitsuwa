...
- ⚠ **実ブラウザ確認は未実施。** 65 工程のガントはこれまでの最大より大きく、
  `LanePacker` の段数と縦の伸び・`main` の横スクロール（Bug #29）は目で見る必要がある
- ⚠ **ガント形式の実ファイルが未取得。** 見出しが揃わないファイルを拒否することは
  合成データで固定してあるが、ANDPAD のガント書き出しそのものでは未確認

### 本番反映の手順（未実施）

⚠ **DB が先・`deploy.sh` が後**（列が無い DB に列を使うコードを乗せると取込画面が 500）。

1. `database/sql/2026-09-01-add-source-to-schedule-steps.sql` を本番へ流す
2. main repo で `composer install --no-dev` → `composer dump-autoload`（新規クラスがある）
3. `./deploy.sh`
4. 本番ブラウザで目視
...
⚠ **ANDPAD 工程表の取込（建売）だけが未反映**（2026-09-01 実装。上記の節を参照）。
DB の ALTER が先・`./deploy.sh` が後。実ブラウザ確認も未実施。
