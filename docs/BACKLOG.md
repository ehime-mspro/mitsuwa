...
- 別タスクとして案内したもの（2026-09-13）: ①論理削除した区画を参照する投資・修繕・問合せ（500・区画が黙って消える）
...
⚠ **サイドバーの開閉の動きは本番では測っていない。** 自動操作したタブが裏（`visibility: hidden`）で、Alpine の x-show の切り替えが setTimeout 経由になり順序が変わるため（Bug #56）。本番に置かれたコードが修正後であることだけ確かめた（動きはローカルの Playwright で確認済み）。⚠ `origin/13.x` への push はしていない（ローカル `13.x` が 68 コミット先行）。
