...
- **Safari での案内の印刷**（1 人 1 ページ・全ページに QR・「ヘッダとフッタをプリント」を入れても余白に日時と URL が出ないか）。
  ⚠ Chrome だけで `<use>` の対応を判断しない
- **本番の実ブラウザ確認**（ローカルは 2026-09-19 に済み）
- **本番反映そのもの**（上記の手順）

---

## ✅ CSV 取込の確認画面から断られると 405 になる件（基幹の取込）— 本番未反映
...
ローカルの実ブラウザ確認（2026-09-19）で見つけた欠陥 8 件（F1〜F8）は **2026-09-24 に直し**、ローカルで確かめ直した。
**本番の実ブラウザ確認は反映の後。**

⚠ **CSV 取込の確認画面から断られると 405 になる件（基幹の取込 4 本）も直し、本番未反映**（上記の節）。approval-phase1 の上に積んであるので、approval-phase1 を出してから早送りする（DB 変更・新規 PHP クラス・依存の変更は無し）。
