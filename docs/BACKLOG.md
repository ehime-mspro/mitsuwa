...
## 周辺ビル調査 第2段の一部（一覧の並び替えと見出しの視認性）— **本番反映待ち**
...
**ブランチ `area-building-sorting` にコミット済み・13.x へは未マージ・本番未反映**（2026-08-29 時点）。
...
⚠ **未反映が 1 件ある**: 周辺ビル調査の並び替えと見出しの視認性（ブランチ `area-building-sorting`）。
コミット済み・全テスト green・ブラウザ確認済みで、**13.x への FF マージと `./deploy.sh` が残っている**。
