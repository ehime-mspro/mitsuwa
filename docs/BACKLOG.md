...
- ⚠ **既知の未修正が 1 件ある（本番反映後に目視で実害を確認すること）** —— 幅 1024px 以上で
  ...
  **本番確認**: `initialPct = 100` になる案件（工程が全部過去）を 1200px 以上で開き
  `scrollLeft === scrollWidth - clientWidth` を測る。ズレたら**呼び出しを
  `requestAnimationFrame(...)` で包むだけ**でよい（関数の中身は不変なのでテストは通る）。
  詳細はプランの §8.1
...
- 表は 6 列。**列幅は実ブラウザで測って決めた**（計画の Task 8）。最小幅 900px は据え置き、割合を各列の
  「中身に要る幅」に比例させた（14.3 / 21.6 / 19.4 / 16.4 / 11.7 / 16.6 %）。
  ⚠ 1200px 幅の表（914px）には最悪ケースの中身（計 926px。7 桁の賃料＋⚠・`-1B1A` など）が 12px 収まらず、
  **1200〜1211px の画面でだけ最悪ケースの行の文字が余白に最大 2.7px 食い込む**（隣の列とは 17px 以上離れる）。
  利用者の判断で許容した（最小幅を上げて表を横スクロールさせる案・余白を詰める案は採らなかった）
