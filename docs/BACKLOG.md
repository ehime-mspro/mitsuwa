...
**未反映のものは無い。** 新規要件は別途追記する。

⚠ **2026-08-31 時点で未実施**: 地図から POI を消した件の**ブラウザ目視 7 点**（上記）。
反映直後の確認では、全ルートが 302（＝アプリは正常）まで見たが、
**302 は認証リダイレクトでビューを描画する前に起きる**ので、
本番でのレンダリング自体はまだ一度も確認していない。
