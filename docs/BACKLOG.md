...
**DB 変更なし・ルート変更なし・新規 PHP クラス（アプリ側）なし**なので `composer dump-autoload` は不要（新しいクラスはテストだけで、本番へは送らない）。
...
ブランチ `date-picker-month-ago` は `month-end-overflow` の先頭から切ってある（記録が衝突しないように）。月末の溢れと一緒に入れる:
...
早送りできるのは 13.x が `e6787c20` のまま（別の作業で進んでいない）とき。進んでいたら 13.x をブランチへマージし、全件を流してから入れる。`month-end-overflow` だけを先に入れた場合も、あとからこのブランチを早送りで入れられる。DB 変更・新規 PHP クラス（アプリ側）は無いので `composer dump-autoload` は不要。

⚠ 反映後の本番確認: 月末の溢れの 4 画面が 200 であることに加え、日付ピッカーの画面を 1 つ開いて「1ヶ月前」を押し、今日の 1 か月前が選ばれることとコンソールのエラーが 0 件であることを見る（月末の溢れる日以外は、選ばれる日は今までと同じ）。
...
**日付ピッカーの「1ヶ月前」（2026-09-24）も実装・検証まで済み、本番未反映**（上記の節。月末の溢れのブランチの上に積んである）。
