...
- 日付ピッカー 8 ビュー（2026-09-23 に数え直し。以前は 6 と書いていた）の `var now = new Date();` は**ブラウザのローカル日**に従う（サーバの日本の今日ではない）
...
**DB 変更・ルート変更・PHP の変更・新規依存はいずれも無し**（Blade の中の JS だけ）。

| 区分 | 実装内容 |
|---|---|
| 対象 | 定義 7 か所・画面 8（DAD 工事案件の登録・編集／住宅の建売・注文住宅の契約の編集／賃貸マンションの部屋・駐車場の改定と解約）|
| 直し方 | `setMonthAgo` の 2 行を「前月の末日（`new Date(年, 月, 0).getDate()`）と今日の日の小さいほう」に。7 か所とも同じ |
...
- テストの死角（docblock に列挙）: `<script>` が描画・実行されるか／コンストラクタで月を足し引きする形
...
`month-end-overflow` だけを先に入れた場合も、あとからこのブランチを早送りで入れられる。DB 変更・新規 PHP クラス（アプリ側）は無いので `composer dump-autoload` は不要。
