...
| 依頼 2 | 3 画面の初回の `update()` を **`document.addEventListener('DOMContentLoaded', update)`** へ（関数と式は不変）|
| テスト | `tests/Feature/Tenant/UnitLabelDisplayTest.php`（15 本）／ `tests/Feature/LayoutMeasuringScriptTest.php`（2 本）。1347 → **1364 tests / 9097 assertions green** |
| ルート / DB | **どちらも変更なし** |
...
- ⚠ **最初の描画は DOMContentLoaded より後**（描画するヘッドレス Chromium で 6 回実測: FCP 76〜96ms / DCL 48〜57ms）
  → 待たせても「一瞬出て消える」は起きない。パース中の計測を残す理由は無い
- **幅・スクロール位置に触るインライン script（11 本）を全件分類するテスト**を置いた（Top trap #13 / Bug #45 ①）。
  新しいビューで幅を測る script が増えると落ちるので、パース中に測っていないかを確かめてから分類して足す

### 範囲外（気づいたが直していない）

- サイドバーの「閉じる」／開くでは `update()` が走らない → 開閉後はヒントが古いまま（**不要なのに出る側にだけ**ずれる）
...
- **変異 34 通りがすべて期待どおり**（カナリア 2 ＋ 依頼 1 の 19 ＋ 依頼 2 の 13。落ちたテストの集合と理由の文言まで照合。依頼 2 のうち等価変異 2 通りは緑）。
  13 か所を旧い書き方に戻す変異は「その画面のテスト＋走査」の 2 本だけが落ち、走査に見えない壊し方 2 通りは画面のテストだけが落ちた
- コンパイル済みビュー **307 本を `php -l`** → INVALID 0 件
- **ローカル実ブラウザ**（使い捨て SQLite ＋ `artisan serve`。zeal 接続も環境変数 `ZEAL_DB_URL` だけで SQLite へ向けた）:
  - 3 画面 × **1024 / 1100 / 1180 / 1280 / 1366 / 1440 / 1800 / 375px** で「スクロールできる ＝ ヒントが見える ＝ フェードが見える」**24/24**・`main` の横スクロール 0
  - 契約 6 画面・物件詳細（契約タブ `3A 1A B1A` ／ 問合せタブ `B1A, 1A` ／ フロアマップ `3F 1F B1F`）・問合せ一覧／詳細・投資登録（`B1A（12.50坪）`）・修繕登録（`B1A 1A 3A A`）で `-1B1A` 0 件
  - ⚠ 非表示のタブではスクロールイベントが配られない（`scrollLeft` は右端まで動くのにハンドラが走らない）。イベントを明示的に送って `is-end` の付け外しを確認した
- ⚠ **本番反映後の目視は別途**
...
**地下区画の表記（`-1B1A` ／ 修繕の `33A` ／ フロアマップの `-1F`）と、3 一覧の横スクロールのヒントの修正（2026-09-12）は本番反映待ち**（上記の節）。
