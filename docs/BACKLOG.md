...
⚠ **Chrome 拡張が未接続だったため、本番の実ブラウザ目視はできていない。**
代わりに**本番のコントローラとビューを実データでレンダリング**して 200 と中身を確認した
（Bug #21 / #26 / #22 / #25 の「本番だけ 500」は retire 済み）。
**見た目の最終確認は人の目で。**
...
⚠ **Chrome 拡張が未接続で本番の実ブラウザ目視だけは未了**（見た目の最終確認は人の目で）。
⚠ 本番の建売は着工予定日・完成予定日とも全行 NULL なので、**64 工程を持つ物件も ◆ は 0 個**のまま。

**未反映のものは無い。** その他の新規要件は別途追記する。
