...
- ⚠ **コンパイル済みビューの `php -l` と、ローカルの実ブラウザ確認はまだ実施していない**（実装計画の Task 8）。
  本番反映の前に済ませること（Bug #21 / #26 / #30 の「本番だけ壊れる」前例）

### 本番反映の手順（未実施）

⚠ **DB 変更・ルート変更は無い**ので SQL は流さない。⚠ ただし**新規 PHP クラスが 1 本ある**
（`app/Support/JapanTime.php`）ので、CLAUDE.md の手順どおり **main repo の cwd で `composer dump-autoload`** を挟む
...
- **基幹の利用者一覧で、自分の最終ログインが「今の日本時間」で出る**（9 時間前になっていない）
- 建売の詳細・不動産の契約の詳細の登録・更新日時、添付の一覧の登録日時も同じくずれていない
- コンパイル済みビューの `php -l` が **INVALID 0 件**（⚠ `view:cache` の成功表示だけでは足りない）
- ⚠ **「今日」のずれが直ったことは、日本時間の 0:00〜8:59 に見ないと確かめられない**
  （9:00 以降は直す前から正しく出るため）
