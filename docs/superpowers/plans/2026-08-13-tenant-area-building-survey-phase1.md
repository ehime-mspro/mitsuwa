...
- Create: `app/Support/FloorNumber.php` ＋ `tests/Unit/Support/FloorNumberTest.php`
  （レビュー C-2。`1F` `B1` `2階` `地下1階` の正規化を 1 箇所に集約）
- Create: `tests/Feature/CdnScriptIntegrityTest.php`（レビュー Mi-3。CDN スクリプトの SRI 分類器）
...
- [ ] **Step 10: ブラウザで実際に取り込めることを確認する（未実施 → Task 13 へ持ち越し）**

⚠ **worktree には本番同等の DB もログイン済みセッションも無いため未実施。**
代わりに画面の `<script>` を node で駆動して実測した（下記「Step 10 の代わりに行った実測」）が、
**SRI 付き `<script>` が実際に読み込まれることはブラウザでしか確かめられない**
（不一致だと無音で読み込まれず取込画面が無反応になる。Bug #28 と同型）。
...
- [ ] **Step 10b: 状態エイリアスの取りこぼしを実データで測る（未実施 — 本番投入後）**
...
### コードレビュー対応（2026-08-17。**テスト 27 → 50 本**、変異 **57 通り**）

仕様適合＝要修正 / コード品質＝Changes requested。**指摘 2 件の Critical は両方クライアント側**で、
**レビュアー 2 人が別々に同じものを見つけた**（サーバ側は既存 3 例より丁寧と評価）。

#### Critical

**C-1: ヘッダー行が「最初の非空行」固定で、画面から変更できなかった。**
1 行目がタイトルの Excel（日本語の業務ファイルでは普通）だと、タイトル文字列が
`name` の guess `/(ビル|…)/` に当たってマッピングが付き、**`name="ビル名"` という架空のビルが
台帳に入る**。しかもプレビューは警告 0 行、結果は「ビル新規 3 件」と**完全成功に見える**。
列マッピングは STEP 2 で直せてもヘッダー行を選ぶ UI が無く、**取込画面の中では修復できない**
（調査回は同一年月スキップがあるので取り直しでも直らない）。設計 §7 は「シート/ヘッダ行選択」を
明記しており、**未実装なのにプラン §1 の逸脱表にも記録が無かった**。
→ `_cost_excel_import` の参照実装（`detectCostHeaderRow` / `populateHeaderRowSelect` /
`onCostHeaderRowChange`）を移植。検出条件は本機能向けに
「**マッピング対象が 2 つ以上ヒットする行**」へ読み替えた（1 つだけならタイトル行でも起こる）。
option は DOM API で動的注入（Bug #16）。

**C-2: 階の列が `1F` `B1` `2階` `地下1階` だと無警告で全部 NULL になっていた。**
`floorLabel()` は B1 = -1 前提で、詳細画面は `orderByDesc('floor')` で並べる。つまり
**Excel でいちばん自然な書き方をすると、取り込んだテナントが全行「—」になり階の並びも消える**
のに、画面はどこにも「捨てた」と言わない（設計 §7.3 違反）。総階数も `10階建` `５階` `地上5階`
が同様。数値検査していたのは件数 3 列だけだった。
→ ①`App\Support\FloorNumber` に正規化を集約（`AreaTenantStatus::fromRawLabel()` と同じ流儀）
②プレビューにも階・総階数・調査年月の警告を足し、**黙って落とす経路をゼロにした**
③非空で読めない / 範囲外は**行ごと弾く**（設計 §7.3 の「行ごと警告に出す」に合わせる）。

#### Important

- **I-1**: 必須マッピングのガードが無く、件数列を割り当て忘れると **0/0/0 の調査回**が入り、
  正しいファイルで取り直しても同一年月スキップで何も起きなかった
  → `goToPreview()` で「ビル名」「件数 3 列のいずれか」を要求（前例と同じ `alert` + 中断）
- **I-2**: 「台帳に無いビルでスキップ N 件」だけが**行数でなく相異なるビル名の数**だった
  （50 行落ちても「1 件」）→ 行数と棟数を両方出す（`… 50 行（3 棟: A / B / C）`）
- **I-3**: 調査年月が空でも送信でき、差し戻されると `back()` のフルリロードで Alpine の state
  （ファイル・シート・マッピング・プレビュー）が全部消える。このビューに `old(` は 0 箇所で
  復元できない。**前例 3 例は親フォームへクライアント側 commit するのでこの往復が無く、
  本画面で新しく生まれた失敗経路**だった → 既定を当月にし、`submitBlockedReason()` で送信を止める。
  ⚠ 理由は**ラッパー `<span :title>`** に置く（disabled なボタン自身の title は出ない。Bug #43）
- **I-4**: ビル取込が N+1（100 棟 × 100 行で **412 クエリ**）→ map にモデルを持ち、調査年月は
  `whereIn` で 1 回先読み。**実測 412 → 202**（内訳 select 2 / update 100 / insert 100 ＝
  残りは実書き込みで、`find()` 100 と `exists()` 100 が消えた）。クエリ数を固定するテストを追加
- **I-5**: テナント明細の再取込が無音で行を倍にし、`divergence()` の乖離警告まで狂う
  → 突合キーの本決めはフォローアップのまま、**画面の注意書き**＋**結果メッセージに
  そのビルの現況テナント総数**を出して気づけるようにした

#### Minor

- `mb_substr` の防波堤が **tenants 入口にしか無く buildings 入口が穴**（Bug #44 の入口×対象）/
  `created_by` / `in:buildings,tenants` / `$map[$key] = $building` / `orderBy('id')` /
  `MAX_ROWS` の境界 — **6 件すべてテストを追加**（レビュー時点では全部 GREEN だった）
- **Mi-1b**: `catch (UniqueConstraintViolationException)` の**コメントが事実と違った**。
  トランザクションで囲んでいないので同一ファイル内の重複は事前チェックが拾い、
  **1 リクエスト内では原理的に到達しない**（実測: 同一ビル・同一月 2 行で `creating` は 1 回）。
  実際に守っているのは**並行リクエストの競合だけ**。コードは正しいのでコメントだけ直した
- **Mi-2**: JS 走査テストを 2 本追加（`payload()` が `okRows()` を通ること / 件数 3 キー
  それぞれに警告があること）
- **Mi-3**: SRI テストが `import.blade.php` しか見ておらず分類器になっていなかった（Bug #45 ①）
  → `tests/Feature/CdnScriptIntegrityTest.php` を新設。**全 `<script src>` を分類**し、
  legacy（9 本で凍結）にも載らず `integrity` も無ければ落ちる。
  ⚠ Google Maps ローダーは**内容が動的でハッシュを固定できない**ので明示的に対象外
  （プラン §1-13 の判断。実測 19 タグ中 9 本が Maps）
- **Mi-4**: ファイルサイズ上限（5 MB）を前例から移植 / **Mi-5**: 「範囲外」を「数値でない」と
  混同しないよう分離。`find()` が null の経路は I-4 の修正で**消滅**（map がモデルを持つ）
- **Mi-6**: `2026年8月度` のような読めない年月が無音で既定月に落ちる → 非空で読めないときだけ警告

#### 直さないもの（判断の記録）

- **Mi-7 `confirmed_on` を取込が設定しない** — 設計 §7 に記述が無い（DDL と
  `validation.attributes` にしか出てこない）。仕様判断が要るので**フォローアップ**として記録
- **Mi-8 規模**（471 → 592 行）— 既存（618 / 1148 行）と比べ妥当。分割しない
- **テナント突合キーの本決め** — 設計に無いのでフォローアップのまま（I-5 は暫定の可視化のみ）
- **既存 9 本の CDN スクリプトへの SRI 後付け** — 設計 §7 の判断でスコープ外。
  `CdnScriptIntegrityTest::LEGACY_WITHOUT_SRI` に**凍結リスト**として記録した

#### 変異の実測（2026-08-17。**57 通り、全て期待どおり**）

実ワークツリーで測定（Bug #50）。**バックアップとの `diff` が非空**であることを毎回確認し、
先頭にカナリアを通した。各変異の前に `php artisan view:clear`。

⚠ **1 回目の測定で 3 件つまずいた。記録しておく:**

| 事象 | 原因 |
|---|---|
| **C1a（detectHeaderRow を 1 行目固定へ戻す）が GREEN** | 構造テストが**関数の存在と呼び出ししか見ていなかった**。名前を残したまま body を戻すと素通りする。→ 波括弧の対応で **body を切り出し**、①複数行走査 ②`hits.length >= 2` ③フォールバック を body の中で固定した。**旧テストでは緑・新テストでは赤**を実測（改善が load-bearing であることの証明。Bug #45 の流儀）|
| **Mi1e が APPLY-FAILED** | `$map[$key] = $building;` が**同じインデントで 2 箇所**にあった（Bug #44 の「狙う行を明示する」）。直前のコメント行込みで指定して解決 |
| **Mi3b が GREEN** | SRI 無しスクリプトを**legacy リストに載っているビュー**へ足していた＝分類器が拾わなくて当然。legacy に無いビューへ足し直して赤を確認 |

| # | 変異 | 結果 |
|---|---|---|
| 0 | カナリア | 赤 |
| C1a–C1d | ヘッダー行: 1 行目固定へ戻す / UI 削除 / `detectHeaderRow()` を通さない / option 注入削除 | 赤 ×4 |
| C2a–C2d | 階: `FloorNumber` を通さない（テナント / 総階数）/ 黙って null に戻す / 警告削除 | 赤 ×4 |
| C2e–C2h | PHP だけ語彙変更 / JS だけ語彙変更 / 接尾辞の順序破壊 / PHP だけ範囲変更 | 赤 ×4 |
| I1 / I2a / I2b | 必須マッピング削除 / 棟数で報告 / 行カウンタ +100 | 赤 ×3 |
| I3a–I3c | 既定月を空に / 月なしで送信可 / 理由をボタン自身の title に | 赤 ×3 |
| I4 / I5a / I5b | 行ごと `find()`+`exists()` / 現況総数を出さない / 画面の注意書き削除 | 赤 ×3 |
| **48a** | **事前チェック削除 ＋ `watchWrites()` アサート除去** | **緑（Bug #48 の再現）** |
| **48b** | **事前チェック削除（アサートは残す）** | **赤** |
| Mi1a–Mi1g | ビル名切詰 / 住所切詰 / `created_by` / `in:` / map 未登録 / `orderBy` 降順 / 境界 `>=` | 赤 ×7 |
| Mi2a / Mi2b | `payload()` が `previewRows` / 件数ループから `unknown` を外す | 赤 ×2 |
| Mi3a–Mi3d | legacy から 1 行削除 / 無 SRI スクリプト追加 / ハッシュ 1 文字 / `crossorigin` 削除 | 赤 ×4 |
| Mi4 / Mi6 | ファイルサイズ上限削除 / 年月の警告削除 | 赤 ×2 |
| R2–R27（18 件） | 初回 29 通りのうちリファクタで死んでいないかの再確認 | 赤 ×18 |

#### クライアントとサーバの一致（Bug #41）

`FloorNumber::parse()` と画面の `areaImportParseFloor()` を **42 入力 × `allowBasement` 2 通り
= 84 ケース**で突き合わせ、**不一致 0**（`scratchpad/sheetjs-t11/{php_table,js_table}.json`）。
語彙と範囲は JSON リテラルで持たせてあり、
`test_floor_vocabulary_matches_between_php_and_js` / `test_limits_match_between_php_and_js` が
**PHP の定数と厳密比較**する（片側だけ変える変異 C2e / C2f / C2h がいずれも赤）。

#### Step 10 の代わりに行った実測（実ブラウザは未実施）

画面に載る `<script>` をそのまま node で駆動した（`scratchpad/sheetjs-t11/drive2.js`）:

- **C-1**: タイトル行つき Excel で `headerRowIndex = 1` を自動検出。ヘッダー行 select に
  4 行分の option（`1行目: 周辺ビル調査表（2026年8月）` …）が出て、手動で 0 行目にも戻せる。
  **架空のビルは 1 件も作られない**（ok 2 行 = アルファビル / ベータビル）
- **I-1**: ビル名未マップ → `step` は 2 のまま ＋ alert / 件数列なし → 同じく中断
- **C-2**: `10階建` OK / `ロビー` → 「階数が読めない」/ `999` → 「階数が 0〜200 の範囲外」/
  `数棟` → 「operating が数値でない」/ `10000` → 「operating が 0〜9999 の範囲外」/
  `45809` → 「調査年月が読めない」。テナント側は `1F` `B1` `地下1階` が通り `PH` `-99` が警告
- **Mi-4**: 6 MB → alert で `step` 1 のまま
- **I-3**: 既定 `surveyedMonth = '2026-08'` / ok 行ありでも月が空なら送信不可 / tenants では月不問
- 前回に続き `payload()` の出力をそのまま実 POST し、DB 反映まで確認

⚠ **残る未確認は前回と同じ**（SRI 付き `<script>` が実際に読み込まれること、ドラッグ&ドロップ、
`x-show` のステップ遷移、コンソールエラー 0 件）。**Task 13 のブラウザ確認へ持ち越し。**

### 修正ラウンドの集中レビュー対応（2026-08-17。**テスト 50 → 55 本**、変異 **49 通り**）

⚠ **前回の修正が、それが直したはずの欠陥を塞ぎきれていなかった。**

#### Critical Cr-1: `detectHeaderRow` が**セル数でなく target キー数**を数えていた

`hits[targets[t].key] = true` を集める内側ループに **`break` が無い**ため、
**1 つのセルが複数 target に同時ヒット**する。日本語の業務ファイルにありがちな複合タイトルは
1 セルで 2 キー以上を踏むので、C-1 の症状（架空のビル `name="ビル名"` が台帳に入り、
警告 0 行・「新規 N 件」で完全成功に見える）が**そのまま再現**した。

⚠ **プランのサンプル `周辺ビル調査表（2026年8月）` が 1 ヒットで済んでいたため通り抜けていた**
——**「動機になった例だけで検証した」型**。次からは**複数の現実的な入力で測る**。

→ `if (targets[t].guess.test(text)) { cells++; break; }`（セル単位で 1 回）。
構造テストの `Object.keys(hits).length >= 2` も `cells >= 2` ＋ `cells++; break;` へ差し替えた。

#### Important

- **I-a**: 語彙の一致テストだけでは **JS の振る舞いを何も守れていなかった**。実測で
  `v = -v;` 削除（`B1` が +1）/ 接尾辞ループの `break` 削除 / `!allowBasement && v < 0` 削除
  が**いずれも 744 本全部緑**（同じ変異を PHP に当てると赤＝非対称）。1 番目は実害があり、
  `B15` がプレビューで +15（警告なし）・サーバで -15（範囲外で拒否）＝ Bug #41 そのもの。
  → C-1 で作った `jsFunctionBody()` を**隣の同型コードへ横展開**し、body 内の分岐と
  3 つの `break` を固定（`test_the_js_floor_parser_keeps_its_load_bearing_branches`）
- **I-b**: `parseMonth` だけが「黙って値を捨てる」経路として残っていた。実測で
  `2026年8月度` / `45809` / `ロビー` の 3 棟が**すべて既定月で登録**され「値が不正でスキップ 0 件」。
  ⚠ `45809` は **`cellDates: true` が失われたときに届く生のシリアル値そのもの**で、
  一度この形で入ると同一年月スキップにより**正しいファイルで取り直しても直らない**。
  → `FloorNumber::parse()` と同じ三値（`null` = 空欄 → 既定月 / `false` = 読めない → 行ごと弾く）
- **I-c**: CDN 分類器の legacy が**ビュー単位**で、凍結対象 9 本（Excel 取込 4 + チャート 4 +
  分析 1 ＝ **次に増える可能性がいちばん高いビュー群**）が将来ぶんも含めて永久に無検査だった
  → キーを **`ビュー|src` の多重集合**にし、消し込みの残りも検査
- **I-d**: 分類器が `src="…"`（二重引用符＋絶対 URL）しか見ていなかった（Bug #45 ③ と同型）。
  実測で**単引用符**・**protocol-relative**・そして最悪の**単引用符の cdnjs**（禁止ホストの
  検査ごとすり抜ける）が全部緑 → `/\bsrc=["'](?:https?:)?\/\/[^"']+["']/i` ＋
  ホスト判定を `//host/…` 対応の正規表現へ
- **I-e**: **Bug #48 の再発**。`watchWrites()` が DB 既存ケースにしか付いておらず（Bug #44 の
  非対称）、`$taken[$monthKey] = true;` を 1 行消すと `creating` が 2 回発火して UNIQUE + catch が
  飲み込み、**応答が完全に同一**になるため 744 本全部が緑だった。docblock が「同一ファイル内の
  重複は `$taken` が拾う」と断言している以上その主張を固定する
  → `watchWrites()` を**回数カウント**に変え、`creates === 1` を固定
- **I-f**: N+1 ガードが buildings 入口にしか無かった（Bug #44「入口ごとに測る」）。
  tenants も同じ 2000 行を受ける → 同型のクエリ数テストを追加

#### Minor（全部対応）

- **48-C**: `catch` を `QueryException` に広げても緑だった → **挙動で**固定
  （`creating` から素の `QueryException` を投げ、握り潰されず伝播することを見る）
- **parity テストの拡張性**: PHP に定数／語彙を足しても気づけなかった →
  `ReflectionClass::getConstants(IS_PUBLIC)` で**キー集合そのもの**を突き合わせ
- **ヘッダー行 select の窓が自動検出と同じ 50 行**で、51 行目以降は手動でも到達できなかった
  （C-1 が解消したはずの「画面の中で修復できない」状態）→ 手動窓を 200 行に分離（検出は 50 のまま）
- **フォールバックの sparse 防御が非対称** → `(r || []).some(...)` に揃えた（`goToPreview` /
  `buildColumns` も同様）
- **空振り防止の下限が現値ちょうど**（19 / 10）→ 15 / 8 に（house style: 実数 33 に対し 30）
- **Bug #43 の後半** `aria-describedby` を実装（理由の `<p>` に id、ボタンから参照）
- **`rows` に `max:3000000`** を追加。根拠は「1 行 ≒ 700 文字 × 2000 行 = 1.4M に 2 倍強の余裕」。
  5 MB のファイルサイズ上限はクライアント側だけで、`MAX_ROWS` の判定は `json_decode` の後
- **範囲定数が 3 系統** → 画面 CRUD 3 コントローラの `validate` を**全出現**突き合わせるテストを追加。
  ⚠ **フォーム HTML の `min=` / `max=` 属性（4 系統目）は未固定**。今は一致している

#### 変異の実測（2026-08-17。**49 通り、全て期待どおり**）

| # | 変異 | 結果 |
|---|---|---|
| 0 | カナリア | 赤 |
| Cr1a | **セル計数の `break` を消す（レビュー再現）** | **赤** |
| Cr1b / Cr1c | 閾値を `>= 1` に / 走査ループを空回りさせる | 赤 ×2 |
| Ia1–Ia5 | `v = -v` 削除 / 接尾辞 break / `!allowBasement` / 二重符号ガード / 地下 break | 赤 ×5 |
| Ib1–Ib3 | `parseMonth` が null を返す / 呼び出し側が弾かない / 範囲チェック削除 | 赤 ×3 |
| Ic1 / Ic2 | **legacy にあるビューへ無 SRI 追加（従来は緑）** / legacy に無いビューへ | 赤 ×2 |
| Id1–Id4 | 単引用符 / protocol-relative / cdnjs / **単引用符の cdnjs** | 赤 ×4 |
| **Ie1** | **`$taken` への記帳を消す（従来は緑）** | **赤** |
| **Ie2** | **同変異 ＋ `creates` アサート除去（対照）** | **緑** |
| Ie3 / If1 | DB 既存ケースの事前チェック削除 / tenants に per-row `find()` | 赤 ×2 |
| Mi48c 他 | `catch` 拡大 / 定数追加 ×2 / 手動窓 / sparse 防御 / aria ×2 / `max:` / CRUD 上限 | 赤 ×9 |
| R-*（18 件） | 前ラウンドの修正がリファクタで死んでいないかの再確認 | 赤 ×18 |

⚠ **今回も 3 件つまずいた（前回と同じ型を 2 つ繰り返した）:**

| 事象 | 原因 |
|---|---|
| **Cr1c（走査ループを空回り）が GREEN** | 構造テストが**トークンの在否しか見ていなかった**。`i < limit` を `i < 0` にしても他の assert は全部残る。→ ループの形（`for (var i = 0; i < limit; i++)` と `limit` の定義）も固定。⚠ **それでも構造テストは「形」しか押さえられない**——振る舞いの正本は node の実測 |
| **MiFb（sparse 防御）が GREEN** | 実装を直したのに**テストを足していなかった**（レビューが Minor に挙げた項目をコードだけ直した）。→ `(r || []).some` を body 内で固定 |
| **MiCrud が APPLY-FAILED → GREEN** | 同じルール行が **store と update に 2 箇所**（Bug #44 を 2 ラウンド連続で踏んだ）。さらに `assertStringContainsString` だと**片方だけ変えても緑**だった → **全出現**を突き合わせる形へ |

#### クライアントとサーバの一致（Bug #41）— 再測定

`FloorNumber::parse()` と `areaImportParseFloor()` を **49 入力 × `allowBasement` 2 通り
= 98 ケース**で突き合わせ、**不一致 0**（`B15` → -15 / `地下15階` → -15 / `5階建て` → 5 /
`Ｂ` → bad / `地上` → bad など）。

#### Cr-1 の修正を**複数の現実的なタイトル**で実測

画面に載る `<script>` を node で駆動（`scratchpad/sheetjs-t11/drive3.js` / `drive4.js`）:

| 1 行目のタイトル | 修正前 | 修正後 |
|---|---|---|
| `ビル別 営業・空き状況調査` | headerRow=0（`name="ビル名"` が入る） | **headerRow=1 / ok=2 / first=アルファビル** |
| `周辺物件 稼働状況一覧` | 同上 | **headerRow=1** |
| `ビル調査（営業状況）` | 同上 | **headerRow=1** |
| `令和8年 周辺ビル空室調査` | 同上 | **headerRow=1** |
| `周辺ビル調査表（2026年8月）` | headerRow=1（たまたま 1 ヒット） | **headerRow=1** |
| tenants `テナント入居状況（アルファビル）` | headerRow=0 | **headerRow=1 / first=アルファビル** |

回帰も確認: タイトル無しの素の表 → `headerRow=0` ✓ / 1 列だけの表 → `headerRow=0` ✓。
**55 行前置き**の表では自動検出は 0（窓 50）だが、**select に 57 行分の option が出て
`56行目: ビル名 / 所在地 / 階数 / 営業` を手動選択でき**、選ぶと `headerRow=55` で正しく取り込めた。

#### 直さないもの（今回の判断）

- **legacy 4 本への SRI 後付け** — ハッシュは同一 URL で実測済みだがコストの問題ではなく、
  **本番稼働中の 4 モジュールを一貫性のためだけに触るリスク**を避ける。Task 13 のブラウザ確認も
  新画面しか見ないため検証されない。**フォローアップのまま**
- **`confirmed_on` / テナント突合キー** — 仕様判断。フォローアップのまま
- **フォーム HTML の `min=` / `max=` 属性（範囲定数の 4 系統目）** — 今は一致。上記のとおり記録のみ
...
### 実施記録 — Step 8〜12 完了（2026-08-17）

| Step | 実測結果 |
|---|---|
| 8 | `13.x` へ FF-merge（`db13ebc5` → `c8a445da`、50 files / 19,788 行追加） |
| 9 | main repo cwd で `composer dump-autoload`（4626 クラス）。新規 11 クラスすべて `class_exists` OK。`autoload_psr4.php` の `$baseDir` は `dirname($vendorDir)` の相対導出で、worktree パスの焼き込み無し |
| 10 | ローカル DB **`masa8787kanri63732`**（⚠ CLAUDE.md の `manage` は誤り）に 3 テーブル。列数 11 / 10 / 12、FK 1 / 2 / 1 |
| 11 | 本番 DB **`mitsuwa-ud_masa8787kanri637`** に 3 テーブル（列数・FK ともローカルと一致）→ `./deploy.sh` 成功 |
| 12 | 下記 |

**Step 10 / 11 の適用方法（プランの当初案は両方とも使えなかった）:**

- `sudo mysql` はパスワードを非対話で渡せない。`DB::unprepared()` はマルチステートメント依存。
  → **`;` で分割して 1 文ずつ `DB::statement()`** で流した。`CREATE TABLE IF NOT EXISTS` 以外が
  混ざっていたら中止する安全ガードを前段に置いている
- 本番は csh なので `ssh <server> /bin/sh` に**ローカルからペイロードを流し込む**。
  PHP は `/usr/local/php/8.3/bin/php`（PATH 上の素の `php` は 7.4）。
  SQL は**実ファイルから機械的に組み立てて** `diff` で一致を確認してから送る（転記ミス防止）

**Step 12 の結果（本番ブラウザ・ログイン済みセッション）:**

| 確認項目 | 結果 |
|---|---|
| 空データで一覧が 200 | ✅ 「周辺ビルのデータがありません。」 |
| 登録フォームの地図とジオコーディング | ✅ 松山市の地図にピン。緯度経度がフォームに反映 |
| **Street View ボタンが出ていない** | ✅ 出ていない（フルスクリーンコントロールも無し） |
| バリデーションが日本語・項目名が画面ラベルと一致 | ✅ 「ビル名は必須です。」「総階数は0以上の値にしてください。」 |
| バリデーション差し戻しがフォームに戻る（Bug #35） | ✅ 生の JSON にならず、`old()` の入力保持も動作 |
| 詳細の乖離警告 | ✅ 実測 5/2/0 に対し明細集計 1/1/0 を並べて警告 |
| Google マップで開く | ✅ `target="_blank"` `rel="noopener"`、`/maps/search/` に `api`+`query` のみ（**キーは含まない**）、座標も一致 |
| キーワードでテナント名検索 | ✅ 在籍中はヒット、**退去済みはヒットしない**。無関係な語も 0 件（判定が効いている対照つき） |
| Excel 取込 | ✅ CSV で実施。**SRI 付き SheetJS が実際に読み込まれた**（`integrity` 属性あり ＋ `XLSX` が定義済み＝ハッシュ不一致ならブラウザが実行を拒否するので、照合と実読み込みの両方が取れる）。ヘッダー行の自動検出・列の自動マッピング・プレビュー・確定まで通し、「ビル新規 3 件 / 調査追加 3 件 / スキップ 0 件」 |
| 座標一括取得 | ✅ 3 件中 2 件成功。**架空住所の 1 件は「未取得」のまま残る**（再課金を防ぐ印が機能） |
| **Bug #43 の再発が無いこと** | ✅ Maps を使えなくして押すと「座標を取得できませんでした（失敗 1 件）」と表示され、**ボタンは `disabled` にならず生きたまま**。無音の成功メッセージも出ない（`df8c00c8` の修正が本番で効いている） |
| 削除モーダル | ✅ 確認モーダルが開き、削除後に一覧へ戻る |
| ページ送りでフィルタ維持 | ⬜ **未実施**。21 件以上必要で本番に大量のテスト行を作ることになるため見送った。自動テスト `test_pagination_keeps_the_keyword_filter` で担保 |

⚠ **Bug #43 の検証を 1 回失敗している。** `delete window.google` した直後に同じ tick で復元したため、
**非同期ループが復元後の `google` を見てしまい、実際には正常系が走った**（＝検証になっていないのに
「動いた」と読めてしまう形）。**非同期処理を止める変異は、処理が終わるまで戻さないこと。**

⚠ **地図の描画確認を早すぎるタイミングで測って「描画されていない」と誤読した。**
`.gm-style` は Map の構築後に生えるので、クリック直後 2.5 秒では 0 件のことがある。
**タイル読み込みを待つか、`.gm-style` の出現をポーリングする。**

**Step 12 で使った Google API の回数（Step 13 の照合用）:**

- Geocoding API: **4 回**（フォームの「マップで確認」1 回 ＋ 一括取得 3 回。
  失敗した架空住所もリクエストとしては 1 回計上される）
- Maps JavaScript API（Dynamic Maps）: **地図の実インスタンス化は 1 回**（登録フォームで
  「マップで確認」を押したときだけ）。一覧・詳細を開いてもローダーは読まれるが地図は作られない

**検証用データは本番から完全に削除済み**（`【検証用】` で始まる 4 棟 / 調査回 4 件 / テナント 3 件を
物理削除。削除前に「検証用以外のビルが 0 件」であることを確認してから実行）。
⚠ SoftDeletes なので**画面からの削除では行が残る**。テスト行を本番に作ったら物理削除まで行うこと。
