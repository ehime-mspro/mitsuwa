...
| 9e | 物件一覧の **入居率だけ** `align="center"` → `align="left"`（賃料収入は据え置き） | `tenant/properties/index.blade.php` | **Task 8 で唯一「赤になるはずが緑」だった穴。** `ParsesSortLinks::thStyleFor()` で列単位に切り出すアサートへ直し、**赤になることを実測**（`Expected 'padding: 0; text-align: center;' / Actual '… left;'` ＝ text-align の話として読める文言） |
| 9f | 部屋一覧の **家賃だけ** `align="center"` → `align="left"`（面積・月額合計は据え置き） | `tenant/units/index.blade.php` | 同上。**Task 8 で緑だった穴 → 列単位のアサート導入後に赤**（`Expected 'padding: 0; text-align: center;' / Actual '… left;'`） |
...
- [x] **Step 4: 確認結果を記録する（2026-08-26 実測）**

⚠ **使い捨ての SQLite ＋ worktree の artisan** で確認した（実 MySQL には一度も到達していない）。
ログインは**パスワードを一切扱わない**よう、一時的な dev 専用ルート（`__preview-login`）を
`routes/web.php` へ追加して使い、**確認後に `git checkout --` で削除**した。

| # | 確認項目 | 結果 |
|---|---|---|
| 1 | 見出しセル全体が押せる | ✅ 物件一覧で**文字から 34px 離れたセル左端**をクリック → 並び替わった。`<a>` が `<th>` を完全に埋めることも実測（`aLeft==left && aRight==right` が 5 列すべて） |
| 2 | 横スクロール中も月額合計が押せる | ✅ `scrollLeft=506`（`sw 1500 / cw 994`）の状態でクリック → `?sort=monthly&dir=desc` へ。**物件フィルタ・`status=`・`keyword=` も維持** |
| 3 | 3 状態が見た目で区別できる | ✅ 未使用 = `aria-sort="none"` / 二重矢印 / `#D1D5DB`、降順 = `descending` / 下向き / 文字 `#047857`・矢印 `#059669`。**3 回目のクリックで `?sort` が消えて既定順へ戻る**ことも確認 |
| 4 | 物件チップを押しても並び順が維持される | ✅ **Alpine のチップをクリック** → `?sort=rent&dir=desc&property_ids[]=1&status=&keyword=`。並び順・絞り込みとも維持。**PHP のテストからは原理的に測れない唯一の経路**（`x-sort-hidden` が Alpine 管理下のフォームでも送信されることの証明）|
| 5 | Tab でフォーカスリングが上辺も含めて見える | ✅ `:focus-visible` が一致し `outline: rgb(5,150,105) solid 2px` / `outline-offset: -2px`。**スクリーンショットで上辺が切れていないことを目視確認**。⚠ `-2px` は「切れるはず」という**推測に対する対策**だったが、実測で妥当だった |
| 6 | 入居率の見出しが列からはみ出さない | ✅ **1024px**: th 95.9 / 必要 96 → はみ出しなし。**375px**: th 83.2 / 必要 83 → はみ出しなし。`main` も `body` も**ページ全体の横スクロールなし**（375 == 375）。⚠ レビューの「約 2px 不足」は机上計算で、**実測では起きなかった** |

⚠ **無効測定を 2 件踏んだので記録する:**
- `document.elementFromPoint()` による当たり判定は **`innerWidth` が 0 の文脈**で走り、
  全部 `null` を返した。**「隅が押せない」という結論を出しかけた**が、これは測定の不備。
  実クリック（`computer`）と `getBoundingClientRect()` で測り直して ✅ を確認した
- 月額合計の**1 回目のクリックが空振り**した。原因は**座標の取り違え**
  （表が 506px スクロールしており、スクリーンショットは縮尺がかかっている）で、
  クリックは隣の「駆除代」に落ちていた。`<a>` が `<th>` を埋めていることは rect で確認済み
