...
- [x] 本番反映は **DB → deploy → 目視** の順で実施（2026-09-01。利用者の明示指示あり）
      —— ALTER 前の実測で `source` なし / **0 行**、`deploy.sh` は exit 0、本番の
      コンパイル済みビュー **266 本 `php -l` で INVALID 0 件**、PHP 8.3.32 で
      `ScheduleImportSheet::read()` が `format=list` を返すことまで確認。詳細は BACKLOG
