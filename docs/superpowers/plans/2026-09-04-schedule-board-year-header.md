...
D14 が「同じ形に揃える」と言っている以上、揃えるのが筋というだけでなく、実害の目もある ——
flex の `min-width` は既定 `auto` で中身の min-content 幅が下限を作るため、2 段から 1 行に
すると min-content 幅が**年と月名の和**（約 45px）に増える。`overflow` が `visible` 以外なら
自動最小サイズが 0 になるので、この下限が消える。
⚠ **現状は到達しない**（`ScheduleCardService` / `ScheduleBoardService` とも軸を
`startOfMonth()`〜`endOfMonth()` に揃えるので、最小の月セルでも約 138px ＝ 45px を大きく上回る。
設計書 §12.3 の実測表）。**将来 `to` が月末に揃っていない軸が来たときの備え**で、
`ScheduleBoardService::headers()` が同じ理由で防御的な三項演算子を残しているのと同じ性質。
