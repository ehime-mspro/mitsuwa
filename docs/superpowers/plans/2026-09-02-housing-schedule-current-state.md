...
- [x] **Step 1: コンパイル済みビューを lint する**（親エージェントが実施済み）
...
**結果（親エージェント実測）: コンパイル済みビュー 266 本 / INVALID 0 件。**

- [x] **Step 2: 旧列名が残っていないことを確かめる**（親エージェントが実施済み）
...
**結果（親エージェント実測）: 残存は 3 種のみで、いずれも無害と確認済み**:
- `database/sql/2026-09-02-...sql` の `CHANGE COLUMN` の**左側**（旧列名から新列名への改名 DDL そのものなので当然残る）
- `HousingConstructionStartDateTest` の**旧列名が「もう無い」ことを確認する否定アサート 7 箇所**
- `HsProperty.php:454` の**歴史的経緯を説明する docblock 1 行**（旧 `actual_completion_date` が現 `construction_start_date` と同じ節目だったという注記）

- [x] **Step 3: 開発サーバを立てて実ブラウザで見る**
...
### 実測結果（2026-09-02、サブエージェントによる実ブラウザ確認）

**環境**: 使い捨て SQLite（`tests/Concerns/CreatesRealEstateSchema` の 19 テーブル + 標準 migration）。
`role=executive` を明示代入したユーザーでログイン。建売物件のメインデモは
**実際に `tests/fixtures/schedule-import/list-format.xlsx`（本番と同じ実ファイル。65件・2シート）を
ブラウザ経由で取込実行**して作成した（file input への直接アップロードは Browser pane では不可のため、
`fetch()` + `FormData` + `document.write()` で実ブラウザの Cookie/CSRF を使った本物の HTTP POST を実行し、
返ってきた実 HTML をそのまま document に差し替えて「内容を確認する」→「この内容で取り込む」を実クリックして確定）。

⚠ **取込直後は今日(2026-09-02)に対して済 15/65（23%）にしかならなかった**（xlsx の工事期間
2026/07/28〜2026/12/25 は今日から見てまだ大半が未来のため）。確認項目1「済が大半」の検証シナリオに
するため、**デモデータの日付のみ一律 -80日シフトした**（**実装コードではなくデモデータの加工**。
分類・1日工程数・備考等の実データ分布は保持）。シフト後: done=52 / running=3 / upcoming=10 /
**1日工程=26件**（実データ「65中26件が1日」という前提と一致）。

| # | 実測結果 |
|---|---|
| 1 | 状態チップ3種とも確認：**これから 10件**（白地/border #D1D5DB, 文字#6B7280）／**進行中 3件**（黒地#111827/白文字）／**済 52件**（bg #F3F4F6, 文字#9CA3AF＝ソースコード既知の低コントラスト仕様どおり）。未定0件（全行に日付あり）。スクリーンショット取得済（375px幅で拡大確認、視認可能） |
| 2 | `box-shadow`を持つ棒＝**3本**。進行中チップの数（3）と完全一致（1:1対応を確認）。「1本だけ」という前提ではなく実データでは3本進行中だったが、リング適用先は過不足なく正確 |
| 3 | 1日工程 **26件**（想定どおり65中26）。全て `width=2.82px`／`backgroundColor: rgb(5,150,105)`（塗り, 透明でない）／`border-radius:4px`。**潰れていない・枠線化していない**ことを実測確認 |
| 4 | `#DC2626` の出現数 **0件**（建売詳細ガント内）。対比として不動産(仕入れ案件)側は同条件で **2件**検出＝住宅だけ正しく遅延バッジが消えている |
| 5 | ◆ **2個**（着工: left=13.52%, 塗り済み rgb(17,24,39) ／完成: left=77.05%, 白抜き rgb(255,255,255)）。着工予定日2026/05/04・完成予定日2026/10/06・今日2026/09/02の位置関係と整合 |
| 6 | **12/12 で `scrollWidth === clientWidth`**：建売詳細(1580/1580, 980/980, 375/375)／注文住宅詳細(1580/1580, 980/980, 375/375)／住宅ボード(1580/1580, 980/980, 375/375)／取込(1580/1580, 980/980, 375/375)。全て等値、横スクロールなし |
| 7 | `[style*="flex: 0 0 262px"]` 該当 **67個**（見出し1＋節目行1＋工程65行）。`getBoundingClientRect().width` は**全て262**（uniqueWidths=[262]）。262以外なし |
| 8 | KPI **3枚**：「進行中の工程 5」「30日以内に始まる工程 14」「30日以内に終わる工程 17」。ステータス絞り込み選択肢：進行中／すべて／これから／済（4種、`<select>`から実測） |
| 9 | 不動産(仕入れ案件)詳細：`#DC2626` **2件**（+20日, +2日の遅延バッジ健在）／`box-shadow`リング **0件**（住宅専用機能が漏れていないことを確認）／編集表ヘッダーに「実績開始」「実績終了」列**あり**。不動産ボード：KPI **4枚**（進行中の案件／遅れている案件／30日以内に始まる工程／30日以内に終わる工程。status=all実測で「遅れている案件1」等の実数値も確認） |
| 10 | 建売詳細・注文住宅詳細とも基本情報が「着工予定日→完成予定日」の順（DOM実測）。新規登録画面(`/housing/properties/create`)にも「着工予定日」ラベルが存在することを`find`で確認 |
| 11 | プレビューに実文字列で2行確認：「**着工予定日を 2026/07/23 にします（現在: —）**」「**完成予定日を 2026/12/25 にします（現在: —）**」。あわせて「取り込み済みの既存の工程 0 件を削除します」「ファイルから 65 件を登録します」「手で追加した工程 0 件は残ります」も実表示を確認 |
| 12 | 4画面とも **「No console logs.」**（各画面ごとに新規タブで測定し汚染を排除。一度 `document.write()` を使った実験タブで無関係な`preload`警告が出たが、新規タブでは0件だったためテスト手法側のアーティファクトと判定・除外） |
| 13 | **既知の制約として記録（直さない。理由は下記）**。着工予定日=2026-01-02（工程群の最早日2026-08-23より約7ヶ月前）に設定した物件で、軸が2025年12月〜2026年10月（11ヶ月）に伸長。5工程の棒は全て left=79.1%〜93.1%の狭い帯に圧縮：基礎工事16.48px／上棟2.05px(1日)／外壁工事22.66px／内装工事28.83px／完了検査2.05px(1日)。1日工程の幅がメイン物件の2.82pxより細い2.05pxまで縮小——「消えては」いないが視覚的圧縮は実在する |
| 14 | **既知の制約として記録（直さない。理由は下記）**。着工予定日=完成予定日=2026-10-02の物件で、2つの◆の`getBoundingClientRect()`が**完全一致**（x=769,y=152,w=16,h=16の同一座標）、2つのラベルの矩形も**完全一致**（x=786,y=151,w=21,h=21）。`overlap()`判定は両方とも`true`。スクリーンショットでも「着工」「完成」の文字が重なって判読不能な状態を確認 |

⚠ **`/dashboard/executive` はデモ DB では 500 になる**（`ms_rooms` 等の賃貸マンションテーブルがデモ用最小スキーマに無いため）。
**デモ環境固有の制約であってアプリの欠陥ではない**（本番 DB には該当テーブルが存在する）。
ログイン成功後は `/housing/*` `/realestate/*` の URL へ直接遷移して確認した。

#### 項目 13・14 は「既知の制約」として記録し、このブランチでは直さない

**項目 14 は親エージェントも独立に再現した**（サブエージェントの実測と座標が完全一致）:

```
着工:  x=827, y=765, w=21, h=16, 親の left=275.93px
完成:  x=827, y=765, w=21, h=16, 親の left=275.93px
```

直さない理由:

1. **ラベル付き ◆ の衝突は、この改修が持ち込んだものではない。** 注文住宅は以前から
   契約 / 完成 / 引渡し の 3 つ、不動産は 契約 / 決済 の 2 つを持っており、同日なら
   同じように重なる。`_schedule_gantt.blade.php` の絶対配置は今回**変更していない**。
   Task 7 は建売でも起こりうるようにしただけ。
2. **軸の伸びは仕様上正しい挙動**（軸はデータの範囲を張る必要がある）。しかも
   **取込後は 着工予定日 = min(planned_start)** なので両者は一致し、食い違うのは
   手で編集した場合だけ。
3. **直すなら設計判断が要る**（ラベルをずらす / 段積みする / 1 つの ◆ に畳んで
   ラベルを結合する）。利用者に諮るべき決定で、このブランチの範囲外。

- [x] **Step 4: 実測結果をプランに追記してコミット**
