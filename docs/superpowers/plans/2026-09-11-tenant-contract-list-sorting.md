...
#### 本番の実データで割合を当て直した（2026-09-11、Task 10 の本番目視の途中）

一度この割合（14.3 / 21.6 / 19.4 / 16.4 / 11.7 / 16.6 %）で本番へ反映したあと、本番の契約 127 件
（「すべて」の全 13 ページ）をログイン済みの実 Chrome から `fetch`（GET・読み取りのみ）で集めて 1 つの表に並べ、
上の検出器で測り直した。**検証用データより長い名前が本番に実在した**:

| 列 | 検証用データ | 本番の実データ | 幅を決めている中身 |
|---|---|---|---|
| 契約日 | 132 | 132 | 日付（同じ） |
| 物件 / 区画 | 200 | **212** | `No.19ミツワビル / 2BCD` |
| 店舗名 | 180 | **207** | 12 字の店舗名 |
| 賃料収入 | 152（7 桁＋⚠） | 139（6 桁＋⚠。⚠ の行は本番に 2 件） | |
| 状態 | 108 | 108 | `解約済み` |
| 操作 | 154 | 154 | 詳細＋編集 |

前回の割合だと、1200px（表 914px）で店舗名が余白に 29px 食い込み**隣の列へ 9px はみ出す**・
表 900px（余白 16px。スマホ・タブレット）でも**隣の列へ 7.7px はみ出す**（1440px では問題なし）。
利用者に確認のうえ、**本番の実データの最大と 7 桁＋⚠ の大きいほう（132 / 212 / 207 / 152 / 108 / 154px、計 965px）**に
比例させた **13.7 / 22.0 / 21.4 / 15.8 / 11.2 / 15.9 %** に当て直した（`a804fd7b`。最小幅 900px は据え置き）。
本番の実データ＋7 桁＋⚠ の 1 行で試算した結果:

| 表の幅（相当する画面） | 余白への食い込み（列ごとの最大） | 隣の列へのはみ出し |
|---|---|---|
| 914px（1200px 幅） | 6.1 / 10.4 / 10.7 / 7.1 / 5.6 / 4.3px | なし（最小でも 9.3px 手前） |
| 1154px（1440px 幅） | なし | なし |
| 900px・余白 16px（スマホ・タブレット） | 0 / 5.5 / 5.7 / 1.3 / −0.4 / 1.4px | なし |

⚠ **教訓: 検証用データの「収まるべき長さ」（物件名 7 字・店舗名 10 字）は本番より短かった。** 列幅は本番の実データで測ること。
全ページを `fetch` して 1 つの表に並べれば、ログイン済みの実 Chrome から**読み取りだけで**測れる
（`X-Requested-With: XMLHttpRequest` を付けてセッションの直前 URL を汚さない。Bug #35。測り終えたら読み込み直して元に戻す）。
