...
> ### ⚠ 実装後のレビューで直したこと（`4cd87c8d`。上のコードはこの修正込みが正）
>
> Chromium の印刷パイプライン（`--headless --print-to-pdf`）で 200 人ぶんを実測して見つかった。
> どれもテストが 1 本も無かったので 6 本足し、5 通りの変異で赤を実測済み。
>
> | # | 直したこと | なぜ |
> |---|---|---|
> | I | 閉じる前の確認を**常に**出す（`afterprint` で外さない） | `afterprint` は「印刷ダイアログを閉じたとき」に発火し、**印刷したのかキャンセルしたのかを JS から区別できない**（主要ブラウザ共通）。一度キャンセルしただけで確認が外れ、100〜200 人ぶんの初期パスワードが二度と出せなくなる |
> | I | `.guide-name` に `overflow-wrap: anywhere` | 空白の無い長い氏名が用紙の外へ**無警告で切れる**（実測: 66 文字で右余白へ 54px、160 文字で完全に切断）。ログイン ID と URL には折り返しがあったのに氏名だけ無かった |
> | I | 有効時間に下限 1 時間 | `Repository::add()` は秒数が 0 以下だと**キーの存在も見ずに** false を返す。`.env` の書き間違いで新規登録も再発行も**1 回目から無言で止まる**（実測） |
> | I | `Cache::add` を使っていることを構造で固定 | 排他性は 1 プロセスの逐次テストでは原理的に測れず、`CACHE_STORE=array` は `add()` の独自実装を持たない（`has` ＋ `put` に書き換えても 14 本すべて緑だった） |
> | M | SVG が 2 つ以上なら例外 | 貪欲な `.*` が最後の `</svg>` まで飲み込み、壊れた中身を黙って返していた |
> | M | 誤り訂正を L → **M** | `<use>` にしたので QR の実体はページ全体で 1 回＝大きさの理由が消えた。紙は折れるしかすれる |
>
> 実測: QR 1 枚 9,887 bytes ／ 200 人ぶんの HTML は **291.9 KB**（毎ページ埋めると 1.9MB）／
> `<use>` 200 個・`<symbol>` 1 個 ／ **PDF 200 ページちょうど**（末尾の空白ページ 0）。
...
>
> ### ⚠ spec 適合レビューで直した 2 点（`c2081631`）
>
> 10. **`store()` にも 1 回限りの鍵が要る。** 「社員番号もメールも一意だから、ブラウザの再送信は
>     必ず `unique` で差し戻る」と判断して付けていなかったが、**すり抜ける窓が実在した** ——
>     登録する → その利用者の社員番号（またはメール）を**編集で変える** → 古いタブで再送信 →
>     `unique` が当たらず、**同じ人の重複アカウントが黙って 1 つ増える**。設計書 §5.12 は
>     4 経路すべてに鍵を要求しているので、例外を認めず挙動を揃えた。
>     ⚠ **claim は `validate()` が通ったあと**・実際に作る直前に置く。先頭で claim すると、
>     氏名の入れ忘れのような**ただの入力エラーで鍵が焼け**、直して送り直せなくなる
>     （差し戻したフォームは同じ鍵を持ったまま再描画される）。2 回目の文言だけ
>     「対象の利用者からパスワードを再発行してください。」に変えた（登録の画面に「再発行」が無いため）。
>     設計書 §5.12 にもこの 2 点を追記した
> 11. **画面の経路で通知メールを一度も見ていなかった。** 再発行のテスト 2 本はどちらも
>     `email => null` の利用者で `Mail` のアサートが 0 件。部品の層の `PasswordReissueTest` は
>     **画面を通らない**ので、`resetPassword()` が `PasswordReissuer` を通らない形に書き換えられても
>     緑のまま（実測: 自前でハッシュを書く変異 M41 が、この 1 本を足す前は全緑）。
>     許可ドメインの利用者で 1 本足し、`Mail::assertQueued` と案内の帯の件数まで見る
>
> ### ⚠ コード品質レビューで直した 13 点（`f00379ba`）
>
> **Critical 1（実装の欠陥）＋ テストの穴 12。** 変異 16 通り中 14 通りが緑だった。
>
> 12. **【Critical】編集モーダルから社長を無効化できた。** 社長を守る歯止めが `update()` には
>     **メールアドレスを空にする経路にしか無く**、`status => inactive` には無かった。
>     `toggleStatus()` / `destroy()` には在ったので、**入口 4 つに対して守りが 3 つ**
>     （Bug #44 の「入口 × 守り」）。画面自身が社長モーダルで「無効化・削除・メールアドレスを
>     空にする操作ができなくなります」と約束していた。無効化された社長は `EnsureUserIsActive` で
>     ログインできないのに**通知メールだけ届き続ける ＝ 決裁が止まる**。
>     文言は 3 つの入口で同一にし、テストは 4 つ目として `test_the_president_is_protected` に足して
>     **`toggleStatus` と同じ文言**まで突き合わせる（+ 編集画面が「無効」を選べることを対で固定）
> 13. **案内に出た初期パスワードが実際に使えることを誰も見ていなかった。** `store()` は平文と
>     ハッシュを**手で組にしている唯一の経路**（再発行は `PasswordReissuer` を通り `Hash::check` が守る）。
>     **別の値をハッシュしても・新規利用者を無効で作っても全緑**＝「印刷して渡した紙でログイン
>     できないアカウント」。→ **案内の HTML から平文を取り出して実際に `/login` する往復テスト 1 本**で
>     両方赤（実測の理由: 「…パスワードが正しくありません。」／「このアカウントは無効になっています。」）
> 14. **決裁の 2 つの印を入れ替えても緑。** `syncApprovalFlags($user, can_view_all, is_admin)` の
>     引数が入れ替わると「全件閲覧者」のつもりが**決裁の管理者**（パスワードも再発行できる）になる。
>     原因は 3 本とも**両方 ON / 両方 OFF / 両方無し**しか作っていないこと → **片方だけ ON を 2 本**
> 15. 社長の指定ルートの**権限が無検査**（`role:executive` を外しても全緑）→ manager / staff で 403 を固定
> 16. **削除済みの利用者を社長にできない、が無検査。** ⚠ `Rule::exists` は SoftDeletes の全域スコープを
>     見ないので、`whereNull('deleted_at')` の 1 行だけが唯一の守り
> 17. **記録の中身が 6 action 中 4 つで件数しか見ていなかった**（old と new を入れ替えても、new を
>     `[]` にしても緑）。監査証跡なので**逆向きに読める記録は無いより悪い** → old / new / `target_type` /
>     `target_id` / `actor_user_id` を固定
> 18. `test_the_role_filter_includes_approval_only` が **Bug #43 の false-pass** —
>     `assertSee('決裁のみ')` が**編集モーダルの `<option>` と説明文**にも一致するので、絞り込みから
>     消しても緑 → 絞り込みの `<select name="role">` **の中だけ**を切り出して見る（`selectHtml()`）
> 19. 検索の**氏名・メールアドレス**とロールの絞り込みが無検査（placeholder は 3 つ約束している）
> 20. **`store()` / `update()` をトランザクションで囲った**（M1）。`attach()` / `sync()` が落ちると
>     **所属部門ゼロの利用者**が残り、案内の紙だけが出る。⚠ **テストでは測れない**
>     （`RefreshDatabase` の入れ子の下で `DB::transaction` を外しても結果が変わらない ＝ 等価変異）
> 21. ルートの並び順の注記が「今 `{user}` に食われる」と**嘘をついていた** —
>     `POST /users/{user}` が無いので現状は食われない（実測で等価）。「将来足したときの保険で、
>     テストでは守れない」に直した（Bug #42 ②）
> 22. 社長の候補が 0 件のとき `<select required>` が**選択肢ゼロの行き止まり**になっていた
>     （社員番号だけで運用する組織で起こりうる）→ 受け皿の文言を出し、押しても何も起きない
>     「設定する」を出さない
> 23. `OneTimeAction` の docblock — 実体は「**同じ鍵の再送信**を止める」だけで、毎回あたらしい
>     40 文字を送れば何度でも通る（`issue()` は発行を記録しない）。security control と読めるので明記
> 24. 社員番号を**空に戻す**経路が無検査（`?? null` を `?? $user->employee_number` にしても緑）
>
> ⚠ **計画から変えたもの**: 再発行・新規登録の `guide_token` は `Str::random(40)` でなく
...
>
> ⚠ **変異の文字列が 2 か所に増えていた**（Bug #44 の「同じ文字列が複数あるときの一括置換」）——
> `store()` にも鍵を足したことで、再発行向けの変異 2 通り（`claim()` の無効化・hidden の削除）が
> **置換 2 箇所で無効測定**になった。差し戻しの文言と行頭の字下げで当て直して両方とも赤。
> **`git diff --stat` の着弾確認だけでなく、置換件数も数えること。**
>
> ⚠ **変異の文字列が動く / 増えることに毎回ぶつかった**（Bug #44 の「同じ文字列が複数あるときの
> 一括置換」）—— `store()` に鍵を足したときに 2 通りが**置換 2 箇所で無効測定**になり、
> トランザクションで囲ったときに 2 通りが**字下げが変わって 0 箇所**になり、門番の変異は
> `Route::middleware('role:executive')->prefix('admin')` が**4 箇所**あって無効になった。
> **`git diff --stat` の着弾確認だけでなく、置換件数も数えること。**
>
> テストは **46 本**（全体 1867 → **1913 tests / 12669 assertions green**）。
> コンパイル済みビュー **272 本を `php -l`** → INVALID 0 件。
> **変異 60 通りすべて赤**（落ちたテストの集合と理由の文言まで照合済み。等価と分かっているのは
> `DB::transaction` を外す 1 通りと、ルートの並び順を入れ替える 1 通りだけで、どちらも上に理由を書いた）。

---
> ### ⚠ Task 10 の検算で分かったこと（`38ece298`）
>
> レビューの直しを別のレビュアーに**測り直させた**ところ、2 件の食い違いが出た。
>
> 1. **「トランザクションはテストで測れない」という申告が誤りだった。**
>    `RefreshDatabase` はセーブポイントを張るので内側のトランザクションは load-bearing で、
>    **記録を書く瞬間**（利用者の保存と所属部門の `attach` の**あと**）で落とせば測れる。
>    実測: 外すと「所属部門ゼロの利用者が残った」「氏名だけ変わって残った」で 1 本ずつ赤。
>    ⚠ **落とす位置が肝**。これより前で落とすと、囲っていなくても何も残らないので検査が成立しない。
>    ⚠ `PasswordReissuer` の docblock の「テストでは原理的に捕まえられない」は
>    **再試行（`transaction($closure, $attempts)`）の話**であって、トランザクション自体の話ではない。
>    そこを読み違えた申告だった。
> 2. **社長を守る 4 入口のうち 2 つ（削除・メールを空に）が文言を見ていなかった。**
>    `assertSessionHas('error')` だけだったので、その入口の文言を変える変異が緑のまま通っていた。
>    ⚠ **文言の 1 か所化（`e52901f4`）が作った穴ではない** — テストは無変更で、
>    逐語コピー時代から同じく素通りしていた（Bug #48 には当たらない）。
>
> ⚠ **変異の作り方でも 1 つ踏んだ**: `DB::transaction(function () {…});` を
> `(function () {…});` に変えるだけでは**閉包を呼ばないのでコードごと壊れ**、
> 無関係なテストも落ちて理由が混ざる。`})();` まで直して**即時実行**にすること。
...
## Task 12 の実測記録（2026-09-17）

### レビューで出た指摘と、その扱い

仕様適合・コード品質の 2 段レビューを回した。**実装コードの欠陥は少なく、
指摘の大半は「テストは緑だが守られていない」型のテスト設計の穴**だった。

直した欠陥（どれも実測で裏を取ってから直した）:

| # | 症状 | 実測 |
|---|---|---|
| 1 | 社員番号 `0` が黙って消える | `LoginId::normalize('0') === '0'` かつ `'0' ?: null === null`。`'0'` は `EMPLOYEE_NUMBER_PATTERN` に当たる ＝ 有効な値。基幹の `normalizedIdentifiers()` は `=== ''` で書かれており、そちらが正しい |
| 2 | 絞り込みが配列（`?search[]=a`）で 500 | `"%{$search}%"` が `Array to string conversion`。⚠ **コントローラだけ直しても防げない** — ビューも `request($key)` を素で出していた（テストが検出）。正規化した `$filters` を渡す形にした |
| 3 | 編集できない相手でも氏名・社員番号の検証が走る | `readonly` な入力欄は値を送るので、古い形の社員番号を持つ人は**唯一許された所属部門の変更までできない** |
| 4 | 断りの文言が基幹と食い違う | §5.7 の実体は「決裁の社長」。`approvalPrivilegeLabel()` は「社長」「全件閲覧者」を返していた。⚠ 呼ぶ側で「決裁の」を足すと「決裁の決裁の管理者」になるのでラベル側で完成させる |
| 5 | §5.9「確認のモーダルに人数と氏名を出す」が未実装 | 素の `confirm()` ではどちらも出せない。計画のコードもそのままで、**計画ごと落ちていた** |

⚠ **`is_string` で絞ると別の欠陥になる。** 配列を弾く guard を `is_string` で書いたら、
整数で渡される `department` が黙って落ちて絞り込みが効かなくなった（既存テストが検出）。
実 HTTP は必ず文字列だが、テストや内部の呼び出しは整数の id を渡せる → **`is_scalar`**。

### 変異テストの実測（26 通り＋当て直し 3 通り）

作法は Bug #44 どおり: ①先にコミット ②各変異の前に `git status --porcelain` が空
③`git diff --stat` が非空で着弾を確認 ④`git checkout --` で戻して空を再確認
⑤**赤/緑でなく落ちた理由まで**突き合わせる。

**検出 24 / 等価変異 2 / 無効な測定 2（当て直して解決）。**

主なもの（すべて期待どおりの 1〜2 本だけが落ちた）:

- 絞り込みの hidden を落とす → 「絞り込んだ全員（N 人）」が絞り込みの外まで再発行（往復テストが検出）
- bulk の `guide_token` hidden を落とす → 1 回目から「すでに実行されました」（同上）
- `@method('PATCH')` / `name="status"` を落とす → 無効化が 405 ／ 差し戻し
- 確認のボタンの `value="selected"` / `"filtered"` を入れ替え → **本数は 2 本のままなので数えるだけでは検出できない**。`x-show` の条件と対で見る形にして検出
- `name="approval_departments[]"` を改名 → 保存のたびに所属が全部消える
- `$reissuableCount` を `$users->total()` に戻す → ボタンの人数と実際の対象が食い違う
- 選べない理由の**画面に見える本文**を消す → タグごと見る形にして検出（素の部分一致だと `title` に当たって緑）

⚠ **無効な測定を 2 件踏んだ（どちらも「赤/緑」だけ見ていたら誤読していた）**:

1. `guide_token` の変異が**当たっていなかった**（`@foreach` との間に Blade コメントがあり
   置換対象が一致しなかった）。`git diff --stat` が空で気づき、当て直して赤を確認
2. `DB::transaction(function () {` を `(function () {` に置換した変異が**赤になったが理由が違った** —
   閉じ括弧が `});` のままで**クロージャが一度も呼ばれない**状態になり、更新系 8 本が落ちていた。
   IIFE（`((function () { … })());`）に直すと**緑** ＝ 期待どおりの等価変異

⚠ **等価変異 2 件（緑が正しい。「守られている」と読み違えないこと）**:

| 変異 | なぜ緑が正しいか |
|---|---|
| `withTimestamps('created_at', false)` → `withPivot('created_at')` | **レビューの前提が誤りだった。** `createAttachRecords()` が `$hasTimestamps = hasPivotColumn(createdAt()) \|\| hasPivotColumn(updatedAt())` で決めるので、**列を挙げた時点で `attach` / `sync` が時刻を書く**（実測: どちらでも値が入る）。`withTimestamps` にしてあるのは意図の明示。**列の宣言ごと落とす**変異は赤になる ＝ テストが守っているのは「時刻が入ること」 |
| `DB::transaction(…)` を外す | 正常系では結果が同じなので、HTTP の応答からは区別が付かない（Bug #48 の型）。守っているのは「一意制約に当たったとき所属だけ変わらない」ことで、その経路は入力チェックが先に弾くため到達させられない |

### 直していないもの（理由つき）

- **更新・状態変更のあと一覧の絞り込みとページが失われる** — 直すには行ごとのフォームに
  絞り込みの hidden を足すことになり、**いま守りを足したばかりの配線の面を広げる**。
  欠陥ではなく使い勝手の話なので、Task 16 のブラウザ確認で評価してから決める
- **検証エラーで編集モーダルの入力が失われる**（`old()` を使っていない）— 同上。
  理由は `$errors` の帯に出るので致命的ではない
- **403/404 の漏れ検査が列挙リスト方式**（`OrganizationManagementTest`）— 今は
  `bootstrap/app.php` の `appendToPriorityList` がグループ全体に効くので**守られている**が、
  「新しいルートが無検査のまま増える」構造。**全件分類へ置き換えるのは Task 15 で行う**
  （Top trap #13 / Bug #45 ①）
...
## Task 16 の実測記録（2026-09-19）

測った対象は `47a34607`（Task 15 の後。作業ツリーは clean のまま）。
使い捨ての SQLite（scratchpad。`migrate` のあと初期データを 1 本のスクリプトで入れた。接続先が scratchpad の
SQLite でなければ止まる安全装置つき）＋ `artisan serve`（127.0.0.1:8765）＋ Playwright の Chrome（画面が見えている状態）。
⚠ ログイン用の使い捨てルートは作っていない（ログインは画面から）。worktree の `public/build` は
`<main repo>/node_modules/.bin/vite build` を cwd=worktree で 1 回流して作った（gitignore 済み）。
初期データ: 基幹の部門 3 ／ 決裁の会社 1・部門 2（RE・SA）・ドメイン `mitsuwat.co.jp` ／ 利用者 7 人
（経営層で決裁の管理者 E0001・基幹の一般担当 S0001・決裁のみで初回ログイン前 A0001・決裁のみの管理者 A0002・
全件閲覧者 A0003・社長 A0004・決裁のみの一般 A0005 / A0006）。

### 結果の要約

- 表の 16 項目: **期待どおり 13 ／ 期待と違った 1（項目 3）／ 未了 1（項目 8: Safari）／ 実装に合わせて読み替えた 1（項目 9）**
- 測っている途中で**欠陥を 8 件**見つけた（下の F1〜F8）。**どれも未修正・判断待ち**。
  テストはどれも緑のまま通っている（F1 の既存テストは `assertRedirect(route('dashboard'))` までしか見ていない）
- コンパイル済みビュー **277 本を `php -l` → INVALID 0 件**（Step 3）
- コンソール: **JavaScript のエラー・警告 0 件**。通信のエラー 3 件は、経営ダッシュボードの 500 ×2
  （検証用 DB に raw SQL 管理の `ms_rooms` が無い。環境の都合）と F2 の 405 ×1

| # | 結果 | 測ったこと |
|---|---|---|
| 1 | ✅ | 375px で欄は 1 つ（「社員番号またはメールアドレス」・`type="text"` `autocapitalize="none"` `autocomplete="username"` `autocorrect="off"` `spellcheck="false"`）。`E0001` でログインできた。小文字の `a0002` でもログインできた |
| 2 | ✅ | `A0006` を誤ったパスワードで 5 回 →「社員番号・メールアドレスまたはパスワードが正しくありません。」、6 回目 →「ログインの試行回数が多すぎます。58秒後にもう一度お試しください。」。6 回とも入力とチェックが残る |
| 3 | ❌ | `A0001` の初回ログイン → パスワード変更の画面 → 変更。**決裁のホームには着くが「決裁以外の画面は使えません。」が出て、「パスワードを変更しました。」は出ない**（F1）|
| 4 | ✅ | `/dashboard/tenant` → 302 → `/approvals` と「決裁以外の画面は使えません。」。ほかに 6 画面（`/tenant/properties` `/admin/users` `/realestate/procurements` `/housing` `/zeal` `/dashboard/executive`）も同じ |
| 5 | ✅ | `A0001`: 展開（1440px）・折りたたみ・ドロワー（375px）とも「決裁のホーム」だけ。`A0002`（決裁のみの管理者）: 展開とドロワーは 3 本、折りたたみは「決」「設」。基幹の `E0001`（決裁の管理者）には基幹のサイドバーの 3 か所とも「決裁の管理」が出る |
| 6 | ✅ | 基幹の新規登録（`S0007`）→ POST の応答がそのまま案内（リダイレクトなし・`Cache-Control: must-revalidate, no-cache, no-store, private`・1 ページ・QR あり）。登録のフォームに初期パスワードの欄は無い |
| 7 | ✅ | 本物のテンプレート（`approvals.login-guide`）を伏せ字のパスワードで 3 人分描画し、Chrome 153 の `--print-to-pdf`（既定でヘッダーとフッターを印刷する）に通した → **A4 で 3 ページ・1 人 1 ページ・全ページに QR・日時や URL なし**。対照として `@page` の余白 0 を外したものは「2026/09/19 2:28」・タイトル・ファイルの URL・「1/3」が印刷された（測り方が効いている）|
| 8 | 未了 | Safari は自動操作できない（computer-use ではブラウザは読むだけ）。利用者の手で確かめる（下の「Safari の確かめ方」）|
| 9 | ✅ | 画面を 1 回押したあとタブを閉じる → 確認が出る（取り消すと画面は残る）。`beforeprint` / `afterprint` を送ったあとも確認が出る。⚠ 計画の「印刷したあとは出ない」は、設計書 §5.12 D9 の「常に確認する」（印刷と取り消しを区別できないため）で置き換わっている。別の URL へ移るとき・「元の画面へ戻る」を押したときも確認が出る |
| 10 | ✅ | 案内から**別の URL** へ移って戻る → `net::ERR_CACHE_MISS`（「フォーム再送信の確認」）。初期パスワードは出ない |
| 11 | ✅ | 基幹の再発行（`/admin/users/9/reset-password`）と決裁の管理者の再発行（`/approvals/admin/users/7/reissue`）の両方で、戻る → 再読み込み（ブラウザが同じ POST を送り直す）→ `302` → 一覧に「この操作はすでに実行されました。案内を印刷し直すには、もう一度再発行してください。」。`approval_setting_logs` の再発行の記録（利用者 9）は 1 行のまま |
| 12 | ✅ | 会社「ミツワテスト」（期の始まり 6 月）→ 部門「総務部」（`ga` → `GA`）→ ドメイン `Example.CO.JP`（→ `example.co.jp`）を登録。編集モーダルを別の行と交互に 5 回開き、`<select>` はすべて正しい値・送信先の ID も切り替わる。部門の編集から保存もできた（F3・F4 は見た目と文言）|
| 13 | ✅ | 権限を持つ 4 人（全件閲覧者・決裁の管理者（決裁のみ）・社長・決裁の管理者（経営層））は選択欄が無い。「—」と操作欄の中心を `elementFromPoint` で押さえると、その `<span>` の `title` に理由が載っている（深さ 0）。一般の 3 人は選択欄あり |
| 14 | ✅ | テンプレート（BOM つき UTF-8・見出し 4 列・見本 2 行）→ 新しい 3 人の CSV → プレビュー「全 3 行・新規 3 人・エラー 0 行」→「取り込む」の確認 →**3 人分の案内**。エラーと注意を混ぜた CSV のプレビューも崩れない（F2 はこの案内の戻るリンク）|
| 15 | ✅ | 6 画面（決裁のホーム・決裁の利用者の管理・部門の管理・CSV 取込・CSV のプレビュー・基幹の利用者管理）× 1800 / 1200 / 375px の **18 通りすべて** `main.scrollWidth === main.clientWidth`（1580 / 980 / 375）|
| 16 | ✅ | JavaScript のエラー・警告 0 件（上の要約）。VERBOSE でパスワード欄の `autocomplete` の提案が出る（下の観察）|

#### 引き継ぎで名指しされた Alpine（Task 11〜14 で一度も動かしていなかった）

- 決裁のドロワー（375px）: × で閉じる・オーバーレイのタップで閉じる。開いている間は `elementFromPoint(16, 26)` が
  ドロワーの見出しの行に当たる（ヘッダーのハンバーガーはドロワーの下に隠れる。基幹のドロワーも同じ作り）ので、
  閉じる手段は × とオーバーレイ
- まとめて再発行: 0 人のときボタンは押せず、包む `<span>` に理由の `title`。2 人選ぶと押せて `title` が消える。
  確認モーダルに人数と 2 人の氏名、見える送信ボタンは 1 つ（`mode=selected`）→ 2 人分の案内。
  「絞り込んだ全員」のモーダルは氏名と「確定のときにもう一度絞り込み直す」の注記を出す（F8）
- 利用者の編集モーダル: 一般の人は氏名・社員番号も編集できる。権限を持つ人は氏名・社員番号が読み取り専用で、
  理由と「決裁の所属部門だけ変えられます。」が出る。その人の所属部門を足して保存できた
- 部門の管理のモーダル（項目 12）

### 見つかった欠陥（未修正・判断待ち）

→ **2026-09-24 に 8 件とも直した**（利用者が「直してから本番へ出す」を選んだ。計画と実測記録は
`docs/superpowers/plans/2026-09-24-approval-phase1-fixes.md`）。直し方は表の案のとおりで、案が 2 つあるものは
F4 = 人数と「届かなくなります」を削除の確認へ移し、行には中立な人数だけ ／ F7 = 再発行のときだけ件数を出す ／
F8 = プルダウンとチェックを変えた瞬間に送る、を採った。F5・F6 は `now('Asia/Tokyo')` / `setTimezone()` ではなく、
13.x で入った `JapanTime::today()` / `JapanTime::format()` を使った（Bug #61）。F1・F2 と同じ形の取りこぼし
（キャンセルの戻り先・ホームの別名・CSV の確認画面からの送り直し）も同じ計画で直した（RULES Bug #63・#64）。

| # | 重さ | 症状（実測）| 原因 | 直し方の案 |
|---|---|---|---|---|
| F1 | **中〜高** | 決裁のみの人がパスワードを変えると、決裁のホームに「**決裁以外の画面は使えません。**」が出て「パスワードを変更しました。」が出ない。**決裁のみの人は全員、初回ログインでこれを踏む**（初期パスワードは必ず変えさせるため）。実測の往復: `302 POST /password/change` → `302 GET /dashboard` → `200 GET /approvals`。⚠ 基幹の人（`S0001`）も `/dashboard` → `/dashboard/tenant` の 2 段で「パスワードを変更しました。」が消える（こちらは以前から）| `PasswordController::change()` が `route('dashboard')` へ戻す。`/dashboard` は決裁のみの人には門番（`RestrictApprovalOnlyUsers`）が警告つきで追い返す画面で、しかも 2 段目の転送でフラッシュが消える | `redirect()->route($user->homeRouteName())`（ログイン直後と同じ行き先の規則）。既存テストの `assertRedirect(route('dashboard'))` を直し、決裁のみの人が変更のあと「パスワードを変更しました。」を見て警告を見ないことを足す |
| F2 | 中 | **CSV 取込の案内の「元の画面へ戻る」を押すと 405 Method Not Allowed**（`GET /approvals/admin/users/import/preview`）。本番（`APP_DEBUG=false`）では英語の素のエラー画面になる。ほかの 4 つの入口（基幹の登録・基幹の再発行・決裁の再発行・まとめて再発行）の戻り先は一覧なので問題なし | 案内の戻るリンクは `url()->previous()` で、これは**リファラーを優先する**。取込の確定は確認画面（POST の応答で、URL は POST 専用の `/import/preview`）から送るので、リファラーが GET で開けない | 案内（`LoginGuide`）へ戻り先を入口ごとに明示的に渡す（CSV は取込の画面か利用者の管理）|
| F3 | 低 | 部門の管理の「略称」が 1440px でも 1 文字ずつ縦に折り返す（「不／動／産」「営／業」）| 見出しの `<th>` は `w-[1%] whitespace-nowrap` なのに中身の `<td>` に `whitespace-nowrap` が無く、列が見出しの幅（2 文字）まで縮む | 略称（とアルファベット）の `<td>` に `whitespace-nowrap` |
| F4 | 低 | ドメインの行に「このドメインのメールアドレスを持つ利用者: 3 人（**この人たちには通知メールが届かなくなります**）」が常に出る。削除の確認と切り離されているので「今届いていない」と読める。削除の確認は「このドメインを削除しますか。」だけ | 設計書 §5.8 は「確認のモーダルに…と出す」。実装は文言を行の横に置いた | 人数と「届かなくなります」を削除の確認へ移し、行には中立な人数だけ（または何も出さない）|
| F5 | 低 | 案内の「発行日」が日本時間の 0:00〜8:59 に**前日の日付**で印刷される（実測: 9 月 19 日 2 時台に「2026年9月18日」）| `LoginGuide` が `now()` を使い、アプリの時刻は UTC。同じ機能の通知メールは `setTimezone('Asia/Tokyo')` している | `now('Asia/Tokyo')` |
| F6 | 低 | 決裁の利用者の管理の「最終ログイン」が UTC のまま（9 時間ずれる）| `last_login_at->format('Y/m/d H:i')`。⚠ **アプリ全体の既存の書き方と同じ**（登録・更新日時・添付など、同じ書き方がこの画面を含めて 20 か所。範囲外）| 決裁の画面の分は `setTimezone('Asia/Tokyo')`。アプリ全体は別の作業で方針を決める |
| F7 | 低 | 案内の帯に、新規登録と CSV 取込でも「通知メール: 送る 0 人／送らない 0 人（メールアドレスなし・許可していないドメイン）」が出る。「1 人分」「3 人分」と合計が合わず、異常に見える | 通知メールを送るのは再発行のときだけ（設計書 §5.13。新規登録で送らないことは §5.10）なのに、帯は件数の行を常に出す | 再発行のときだけ件数を出す（または「新しく登録した人には通知メールを送りません」と出す）|
| F8 | 低〜中 | 絞り込みのプルダウンを変えて「検索」を押さないまま「絞り込んだ全員（N 人）を再発行」を押すと、**画面の表示（例: 営業部）ではなく、前回適用した絞り込み（例: 全員 6 人）が対象**になる。確認モーダルに対象の氏名は出るので、読めば気づける | 絞り込みは「検索」ボタンで送る作り（基幹の利用者管理と同じ）で、まとめて再発行の hidden は適用済みの条件を持つ | プルダウンとチェックを変えたら即時に送る（`CLAUDE.md` の一覧の規約「即時フィルタ」）か、モーダルに適用中の条件を出す |

### 観察（欠陥ではない・記録だけ）

- 1440px の決裁の利用者の管理で、見出し「決裁の所属部門」が「決裁の所属部／門」、印の注記が「…されていま／す」と
  1 文字だけ折り返す（見た目だけ）
- パスワード欄に `autocomplete="current-password"`（ログイン）/ `"new-password"`（変更）が無い（Chrome の VERBOSE。
  パスワード管理ソフトの使い勝手の話）
- 取込の行エラーは 1 行につき最初の 1 つだけ出す（アプリの取込に共通の作り。`$rowErrors[] = …; continue;`）。
  メールと部門の両方が誤っている行は、直して上げ直すと 2 つ目が出る
- テンプレートの見本の行は社員番号 `A0001` / `A0002`。本番に同じ番号の人がいて見本のまま上げると「更新」になる
  （確認画面に氏名の違いの注意が出るので気づける）

### Safari の確かめ方（項目 8。利用者の手で）

1. 案内を出す（基幹の利用者管理で誰かを再発行するか、検証用のデータで新規登録する）
2. Safari の「ファイル → プリント」で、**1 人 1 ページ**・**全ページに QR**（2 ページ目以降にも出るか。`<use>` の対応）・
   余白に日時や URL が出ないことを見る。⚠ Safari は印刷ダイアログの「ヘッダとフッタをプリント」を**入れた状態でも**見る

### 測り方の注意（次に同じことをする人向け）

- ⚠ Playwright MCP の `browser_run_code` は、確認ダイアログ（`confirm` / `beforeunload`）が出た瞬間に
  「Modal state」を返して**コードの戻り値を捨てる**。押す操作は `click({ noWaitAfter: true })` で終え、
  `browser_handle_dialog` で受けてから次を測る
- ⚠ `browser_run_code` の中では `require` が使えない（ファイルを書けない）。ダウンロードは `download.saveAs()` で
  `.playwright-mcp/` の下へ（MCP も別名で 1 つ保存する）。アップロードは `setInputFiles()`
- ⚠ 印刷の確認に本物の Chrome の `--headless=new --print-to-pdf` を使うと、**PDF を書き終えても終了しない**
  （2 分待っても残った）。裏で起動し、PDF の大きさが止まったら止める。**対照（余白 0 を外したもの）を必ず一緒に印刷する**
  —— 対照に日時と URL が出て初めて「出ない」が測れたことになる
- ⚠ 案内から**同じ URL** へ移ると（例: 基幹の登録の案内 →「元の画面へ戻る」→ `/admin/users`）、Chrome は履歴を
  置き換えるので「戻る」が案内を飛ばす。項目 10 は**別の URL** へ移ってから戻る
- ⚠ `url()->previous()` はセッションの直前の URL より**リファラーを優先する**（F2 の原因）
- 経営ダッシュボードは検証用 DB（migration だけ）に raw SQL 管理の表が無いので 500 になる。
  経営層でログインしたら、確かめたい画面の URL を直接開く
