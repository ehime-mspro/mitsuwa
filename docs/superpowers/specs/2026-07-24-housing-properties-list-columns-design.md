...
### 3.6 合計は表示内訳からの積み上げ（お客様所有土地でも整合）【2026-07-24 final review で方針転換】

**合計（販売金額・原価額・粗利額）は `getSellingPriceTotal()` / `getTotalCost()` / `getGrossProfit()`
を直接呼ばない。** 画面に実際に表示している建物・土地の値（`isCompanyLand()` ガード後）から
積み上げる — 詳細画面 `properties/show.blade.php` が採る「表示値の積み上げ」と同じ思想。

```php
// 建物・土地を先に計算（土地は isCompanyLand() ガード後）
...
// 合計は建物＋土地の積み上げ（両方 null のときだけ合計も null）
...
合計の税込サブ行も、ガードなしの合計を返す `getSellingPriceTotalWithTax()` ではなく
**`$tPrice + $bTax`**（表示している合計＋建物消費税）を使う。建物の税込サブ行は表示値と
完全一致するため従来どおり `getBuildingSellingPriceWithTax()` のまま。

**なぜ変えたか**: 旧方式（既存メソッド直呼び）は `land_source_type` が `customer_land` や
`NULL`（未選択）でも、`getTotalCost()` / `getGrossProfit()` が生カラムの `land_cost` を無条件で
合算してしまい、土地列は「—」なのに合計だけ土地原価が乗る不整合があった。
`makeCustomerLand()` フィクスチャ（building 32,000,000 / 24,800,000・land_cost 9,600,000
・お客様所有土地）で実測すると、旧方式は合計原価 34,400,000円・合計粗利 **-2,400,000円(赤)**
になる一方、建物粗利は **+7,200,000円(緑)** — 同一行に緑と赤の粗利が並ぶ矛盾した表示になっていた。
積み上げ化後はどちらも 24,800,000円 / +7,200,000円で一致し、`land_source_type` の値に関わらず
「合計＝見えている建物＋土地」が常に成り立つ。

**共有モデルメソッドは無変更**: `getSellingPriceTotal()` / `getTotalCost()` / `getGrossProfit()` 自体は
削除・変更しない（`properties/show.blade.php` など他の呼び出し元に影響を与えないため）。
一覧 `<tbody>` の `@php` ブロックだけが、これらを呼ぶ代わりに表示値（`$bPrice`/`$bCost`/…）から
積み上げる — Model は触らない設計のまま。
...
| ~~合計を建物・土地の表示値から積み上げ直す~~ **→ final review 指摘で採用済み（§3.6）** | 当初は「共有メソッドの挙動を変えて詳細画面に波及する」ことを懸念してスコープ外にしたが、実際には一覧の `@php` ブロックが `getSellingPriceTotal()` 等を**呼ぶのをやめるだけ**でメソッド自体は無改変のため、他の呼び出し元（詳細画面等）には影響しないと判明。方針転換し実装済み |
...
| 4 | **【解消済み】** お客様所有土地 × 契約ありの不整合は §3.6 の積み上げ化で構造的に根絶したため、デプロイ前に本番で「お客様所有土地の建売が 0 件」を確認する作業は不要になった（`land_source_type` の値に関わらず合計＝表示内訳で常に整合する） |
