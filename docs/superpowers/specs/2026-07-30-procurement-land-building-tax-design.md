...
1. ~~**税込入力の往復で 1 円ずれることがある。**~~
   **【2026-07-31 大幅に改善】** 逆算（税込 → 税抜）を**切り上げ**に変更した。
   当初「原理的に避けられない」と書いたが、**それは切り捨てにしていたから**で、
   切り上げれば往復は数学的に可能な限り一致する。実測（1,000,000〜100,000,000 を掃引）:
   ...
   ⚠ **「常に +1」ではなく切り上げ。** 割り切れるときに +1 すると壊れる
   （税込 11,000,000 は税抜ちょうど 10,000,000 なのに 10,000,001 → 税込 11,000,001）。

   ⚠ **残る不一致は全件が「到達不能な税込」**（10% で 11 分の 1）。税額を切り捨てる以上
   `incl(E) = E + floor(E × 税率)` は E が 1 増えるごとに 1 か 2 増えるため、
   **どの税抜からも作れない税込が構造的に存在する**（20,010,000 は 18,190,909 → 20,009,999 と
   18,190,910 → 20,010,001 の間で作れない）。丸め方向を変えても位置が動くだけで消せない。
   契約側は `tax_amount` の手入力で実額に合わせられる。

   ⚠ **実際に効くのは JavaScript 側。** 税込入力欄は `name` 属性を持たず送信されないため、
   サーバは逆算を実行しない（`ConsumptionTax::toExclusive()` はテストからしか呼ばれない）。
   PHP だけ直しても画面は変わらないので、Blade の Alpine 3 箇所と対で直すこと。
   走査テスト `tests/Feature/RealEstate/TaxExclusiveCeilingJsTest.php` が
   `Math.floor` への逆戻りと、ついでに税額まで切り上げてしまう誤りの両方を検出する
   （4 通りの変異で赤になることを実測済み）。
