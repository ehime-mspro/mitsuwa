...
⚠ **追記（2026-09-11 実装時の実測）: 上の判定式 `scrollWidth <= clientWidth` は余白の内側への食い込みを検出できない。**
td の `scrollWidth` が増えるのは文字がセルの外枠（パディングの外）まで出たときだけで、
実際に 1200px で 17.2px 食い込んでいたセルも `scrollWidth === clientWidth` だった。
文字の範囲（`Range`）をセルの内容幅・外枠と比べて測った。検出器の全文と実測値は計画の「Task 8 の実測結果」。
**決まった値**: 最小幅 900px ・割合 13.7 / 22.0 / 21.4 / 15.8 / 11.2 / 15.9 %（各列の中身に要る幅に比例）。
方針は「最小幅はそのまま、割合を中身に比例させる」（利用者の判断。最小幅を上げてその幅で表を横スクロールさせる案・
余白を 16px に詰める案は採らなかった）。
⚠ 最初は検証用データの幅（計 926px）で 14.3 / 21.6 / 19.4 / 16.4 / 11.7 / 16.6 % にしたが、**本番の実データは
検証用データより長い名前を含んでいた**（物件 / 区画 212px・店舗名 207px）。本番で測り直し、実データの最大と
7 桁の賃料＋⚠ の大きいほう（計 965px）で当て直した。1200px 幅の表（914px）では最悪ケースの行の文字が余白に最大 10.7px、
スマホ・タブレット（表 900px・余白 16px）では最大 5.7px 食い込むが、**どの幅でも隣の列へははみ出さない**。
