...
| 空室率 | 全て / 満室(0%) / 1%以上 / 20%以上 / 40%以上 | 「全て」は空値 |
...
2. **位置地図** — 緯度経度があれば表示。無ければ「位置未登録」の案内と編集への導線
...
- ピンクリックで InfoWindow（ビル名 / 営業・空き・不明 / 空室率 / 詳細へのリンク）
...
このページのスクリプトタグにだけ `&libraries=places` を足す。各ページが個別に script タグを
持つ構成なので、既存 8 画面には影響しない。

ビル名を入力すると候補が出て、選ぶと `address` / `latitude` / `longitude` が埋まる。
検索は愛媛県周辺にバイアスをかける。**有料契約が前提**（リクエスト課金）。
...
- 既存 16 棟は住所からの一括ジオコーディングで下地を作り、ズレだけ手で直す
...
3 テーブル / Enum / モデル / 一覧（表のみ）/ 詳細 / ビル登録編集フォーム（ジオコーディング＋ドラッグピン）/
調査の追加編集画面 / テナントの追加編集画面 / Excel 取込（ビル＋調査、テナント明細）/ サイドバー。

**この時点で既存 Excel を流し込んで運用を開始できる。**

### 第2段 — 見える化

一覧地図（色分け・自社物件重ね）/ Places 補完 / `properties` への緯度経度追加と物件編集画面の地図 /
エリア集計と空室率の推移。
...
6. 本番ブラウザで確認 — **一覧のフィルタ「全て」→ 2 ページ目、詳細の乖離警告、地図の表示**を実際に触る
