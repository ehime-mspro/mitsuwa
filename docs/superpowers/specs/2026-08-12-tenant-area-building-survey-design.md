...
- ピンクリックで InfoWindow（ビル名 / 総階数 / 営業・空き・不明 / **入居率** / 空室率 / 最終調査 / 詳細へのリンク）
  ...
  > **値は「100 − 空室率の整数」。** private な `VacancyRate::compactPercent()` が返す
  > **1 つの整数**から空室率側（`compactLabel()`）と入居率側（`occupancyCompactLabel()`）の
  > 両方を作る。`ceil(100 − v) === 100 − floor(v)` なので
  > **2 つの整数の和が必ず 100 になる**（構造として保証。テストで見張るのではない）。
  > ⚠ 片方が他方の**表示文字列**を解釈する形にしない —— 接頭辞が付くと `(int)` が 0 と読んで
  > 無音で `100%` を返し、番兵 `'—'` を変えると未調査の棟が満室の緑丸で出る（2026-08-20 実測）。
