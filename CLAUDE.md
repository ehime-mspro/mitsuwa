...
| 17 | ページの `<script>` で幅・スクロール位置を**パース中に**測る（Alpine（defer）より前に走り、PC サイドバーが `x-cloak` で隠れているので表示領域が **220px 広い**。1024px 以上のある幅でだけ「スクロールできるのにヒントが出ない」「初期スクロールが 220px 手前で止まる」になり、テストも `view:cache` も HTML も全部正しい）| 初回の計測は `document.addEventListener('DOMContentLoaded', …)` の中で。⚠ `requestAnimationFrame` は不可（非表示のタブで止まり、Alpine より後に走る保証も無い）。⚠ **1024px 未満では再現しない**ので、確かめるときは 1024px 以上の実ブラウザで幅を変えて測る。走査テスト `LayoutMeasuringScriptTest` が幅・スクロール位置に触るインライン script を全件分類で拾う（新しいビューが増えたら分類して足す）。Bug #56 |
