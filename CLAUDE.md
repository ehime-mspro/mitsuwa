...
| 7 | Tailwind の新規クラス追加（Vite ビルド済 CSS にしか含まれないクラスは効かない）| 既存クラス確認 `grep ".my-class" public/build/assets/app-*.css`、または inline style |
...
| **frontend-design** | UI を新規作成する時 | ⚠ **Vite ビルド済 Tailwind の制約あり**。生成された新規 Tailwind クラスは `public/build/assets/app-*.css` に無いと無効化される。生成後は (1) `grep '.<class>' public/build/assets/app-*.css` で確認 → (2) 無いクラスは inline style に書き換え。任意値（`min-w-[140px]` 等）は必ず inline style。Bug #19 |
