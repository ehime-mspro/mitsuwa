...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //    ⚠ **順序を誤っても「送り直せなくなる」画面は 1 つも無い**（design doc §5.12・
        //    2026-09-17 Task 13 実測。CSV の確定でさえ同じ鍵を再掲しない —— `preview()` が
        //    毎回 `issue()` し直し、確定を断られた差し戻し先の GET /users/import には
        //    guide_token も csv_data も無い）。誤った順序で焼けるのは、どのみち**捨てられる側の
        //    鍵が 1 つ増えるだけ**。それでも検証の後に置くのは、**起きなかった処理に鍵を焼いては
        //    いけない**という §5.12 の決まりそのもののため（この画面は hidden が描画のたびに
        //    `issue()` を呼び、検証エラーは `back()` ＝ 一覧を GET し直すので毎回あたらしい
        //    鍵になる。実測）。
