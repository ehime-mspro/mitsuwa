...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // ⚠ **検証が通ってから**鍵を使う（設計書 §5.12）。
        //    ⚠ **順序を誤っても「送り直せなくなる」画面は、通常の遷移では 1 つも無い**
        //    （design doc §5.12・2026-09-17 Task 13 実測。CSV の確定でさえ同じ鍵を再掲しない ——
        //    `preview()` が毎回 `issue()` し直し、確定を断られた差し戻し先の GET /users/import
        //    には guide_token も csv_data も無い）。誤った順序で焼けるのは、どのみち**捨てられる
        //    側の鍵が 1 つ増えるだけ**。それでも検証の後に置くのは、**起きなかった処理に鍵を
        //    焼いてはいけない**という §5.12 の決まりそのもののため（この画面は hidden が
        //    描画のたびに `issue()` を呼び、検証エラーは `back()` ＝ 一覧を GET し直すので
        //    毎回あたらしい鍵になる。実測）。
        //    ⚠ **Task 13 の実測は通常の遷移（差し戻し → 再送信）だけを見ている** —— ブラウザの
        //    「戻る」で送信前の古いページ（古い guide_token を hidden に持つ）へ復元し、
        //    そこから送るような経路までは検証していない。
