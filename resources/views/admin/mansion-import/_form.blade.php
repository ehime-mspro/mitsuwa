{{-- 各タブ共通のインポートフォーム（賃貸マンション） --}}
{{-- 必要変数: $tab, $actionUrl, $templateUrl, $fileInputName, $fileNamesKey --}}
        ...
        <div style="flex: 1;">
            ...
            <div style="display: flex; align-items: center; gap: 12px;">
                <label style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border: 2px dashed #d1d5db; border-radius: 8px; cursor: pointer; font-size: 13px; color: #6b7280;">
                    ...
                    ファイルを選択
                    <input type="file" name="csv_file" accept=".csv,.txt" style="display: none;" x-on:change="onFileSelect($event, '{{ $fileNamesKey }}')">
                </label>
                <span x-show="fileNames['{{ $fileNamesKey }}']" style="font-size: 13px; color: #059669; font-weight: 600;" x-text="fileNames['{{ $fileNamesKey }}']"></span>
            </div>
