...
@section('title', '原価項目マスター')

@section('breadcrumb')
    <span class="mx-1.5">›</span>
    <span>システム管理</span>
    <span class="mx-1.5">›</span>
    <span class="text-gray-600">原価項目マスター</span>
@endsection

@section('content')
<div x-data="costItemManager()">

    {{-- ページタイトル --}}
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-lg max-lg:text-base font-bold text-gray-900">原価項目マスター</h1>
    ...
    {{-- 削除確認メッセージ --}}
    <div x-show="deletingId !== null" class="mt-3 rounded-lg border border-red-200 bg-red-50 p-3" style="max-width: 560px;">
        <p class="text-sm text-red-800">「<span x-text="deletingName"></span>」を削除しますか？ この操作は取り消せません。</p>
        <p class="text-xs text-red-600 mt-1">※ この項目を使用している原価明細がある場合、削除できません。</p>
    </div>
    ...
    {{-- 非表示フォーム --}}
    <form x-ref="addForm" method="POST" action="{{ route('admin.master.re-cost-items.store') }}" class="hidden">
        @csrf
...
<script>
function costItemManager() {
    return {
        items: @json($costItemsForJs),
        editingId: null,
        editingName: '',
        deletingId: null,
        deletingName: '',
        ...
        dragOverIndex: null,
        reorderMessage: '',
        baseUrl: '{{ url("/admin/master/re-cost-items") }}',
