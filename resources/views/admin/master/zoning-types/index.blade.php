...
@section('title', '用途地域マスター')

@section('breadcrumb')
    <span class="mx-1.5">›</span>
    <span>システム管理</span>
    <span class="mx-1.5">›</span>
    <span class="text-gray-600">用途地域マスター</span>
@endsection

@section('content')
<div x-data="zoningTypeManager()">

    {{-- ページタイトル --}}
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-lg max-lg:text-base font-bold text-gray-900">用途地域マスター</h1>
        ...
        <table class="w-full" style="table-layout: fixed;">
            ...
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="text-xs font-semibold text-gray-600 text-left py-2.5 px-3">用途地域名</th>
            ...
            <tbody>
                ...
                <tr x-show="adding" class="border-b border-gray-100 bg-emerald-50/30">
                    ...
                    <td class="py-2.5 px-3">
                        <input type="text" x-model="newName" placeholder="用途地域名を入力"
                               class="w-full px-3 border border-emerald-400 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:outline-none"
                               style="height:36px;"
                               x-ref="newNameInput"
    ...
    <div x-show="deletingId !== null" class="mt-3 rounded-lg border border-red-200 bg-red-50 p-3">
        ...
        <p class="text-xs text-red-600 mt-1">※ この用途地域を使用している不動産案件がある場合、削除できません。</p>
    </div>

    {{-- フッター情報 --}}
    <div class="mt-4 text-xs text-gray-400">
        <p>全 <span x-text="items.length"></span> 件 — 不動産案件登録・編集画面のセレクトボックスに上記の順で表示されます。</p>
    </div>

    {{-- 非表示フォーム: 新規追加 --}}
    <form x-ref="addForm" method="POST" action="{{ route('admin.master.zoning-types.store') }}" class="hidden">
        @csrf
...
<script>
function zoningTypeManager() {
    return {
        items: @json($zoningTypesForJs),
        editingId: null,
        editingName: '',
        deletingId: null,
        deletingName: '',
        ...
        dragOverIndex: null,
        reorderMessage: '',
        baseUrl: '{{ url("/admin/master/zoning-types") }}',
