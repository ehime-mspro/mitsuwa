...
@section('content')
    ...
    @php
        ...
        $statusOptions = collect(\App\Enums\ProjectStatus::cases())->map(function ($s) {
            return [
                'value'       => $s->value,
                'label'       => $s->label(),
        ...
        <div style="overflow-x: auto;">
            <table class="w-full border-collapse">
                ...
                <tbody>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            @if($canEditStatus)
                                <td class="px-3 py-3 border-b border-gray-100 text-center whitespace-nowrap"
                                    x-data="projectStatusCell({{ $pj->id }}, '{{ $pj->status->value }}', '{{ $pj->status->label() }}', '{{ $pj->status->badgeClass() }}')">
                            ...
                            <td class="px-3 py-3 border-b border-gray-100 text-center whitespace-nowrap">
                                @if($pj->latitude && $pj->longitude)
                                    <button type="button" onclick="openMapModal({{ \Illuminate\Support\Js::from($pj->project_name) }}, {{ \Illuminate\Support\Js::from($pj->address) }}, {{ $pj->latitude }}, {{ $pj->longitude }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-10 text-center text-sm text-gray-400">分譲地データがありません。</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
window.__projectStatusOptions = @json($statusOptions);

function projectStatusCell(id, initialValue, initialLabel, initialBadgeClass) {
    return {
        id: id,
        value: initialValue,
        label: initialLabel,
        ...
        popoverTop: 0,
        popoverLeft: 0,
        options: window.__projectStatusOptions || [],
        ...
        select: function(opt) {
            ...
            fetch('{{ url("/realestate/projects") }}/' + self.id + '/status', {
                method: 'PATCH',
                headers: {
