...
{{-- 判定はパース中と DOMContentLoaded の 2 回（2026-09-13）。パース中の判定でその後の描画を決め、
     Alpine の起動後にもう一度測って直す。
     ⚠ この script より前（表の途中など）で描画が挟まると、その 1 回だけ既定の「表示」が写ることがある
       （300ms 遅延で 20 回中 1 回。既定が「表示」なので避けられず、修正前から同じ）。
