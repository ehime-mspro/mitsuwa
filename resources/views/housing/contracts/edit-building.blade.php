...
<div class="hc-edit-wrapper">
    ...
    <form method="POST" action="{{ route('housing.contracts.update-building', $hsContract) }}"
                ...
                <div class="hc-field">
                    ...
                    <select name="created_by" class="hc-select">
                        ...
                        @foreach($staffUsers as $user)
                            <option value="{{ $user->id }}" @selected(old('created_by', $hsContract->created_by) == $user->id)>{{ $user->name }}@if($user->trashed())（削除済み）@elseif($user->status === \App\Enums\UserStatus::Inactive)（無効）@endif</option>
                        @endforeach
                    </select>
        ...
        <x-form-actions submit-label="更新する" :cancel-url="route('housing.contracts.show-building', $hsContract)" />
    </form>
