...
@section('breadcrumb')
    ...
    <span class="mx-1.5">›</span>
    <span class="text-gray-600">テナントを編集</span>
@endsection
...
@section('content')
    ...
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900 mb-4">{{ $building->name }} — テナントを編集</h1>
    ...
    {{-- ⚠ 「保存して続けて登録」は編集画面には出さない（設計 §5.6） --}}
    <form method="POST" action="{{ route('tenant.area-buildings.tenants.update', [$building, $tenant]) }}">
        @csrf
        @method('PUT')
