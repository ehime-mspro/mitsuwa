...
function inquiryCreateForm() {
    return {
        ...
        checkedUnits: @json(old('unit_ids', [])).map(Number),
...
<div x-data="inquiryCreateForm()">
    ...
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900 mb-4">問合せ 新規登録</h1>
    ...
    <form method="POST" action="{{ route('tenant.inquiries.store') }}">
        @csrf
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="date" name="inquiry_date" value="{{ old('inquiry_date', \App\Support\JapanTime::today()->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <select name="property_id" x-model="propertyId" @change="checkedUnits = []"
                ...
                <div class="sm:col-span-2">
                    ...
                    <div x-show="!propertyId" style="display:none;" class="text-sm text-gray-400 py-2">物件を選択してください</div>
                    <div x-show="propertyId && filteredUnits.length === 0" style="display:none;" class="text-sm text-red-600 bg-red-50 border border-red-200 rounded-md p-3">現在空室がありません</div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    ...
                    <select name="desired_usage_id"
                        ...
                        @foreach($usageTypes as $ut)
                            <option value="{{ $ut->id }}" {{ old('desired_usage_id') == $ut->id ? 'selected' : '' }}>{{ $ut->name }}</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">※ 選択肢はマスター設定で管理</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="month" name="desired_move_date" value="{{ old('desired_move_date') }}"
                           class="form-input w-full h-[40px] px-3 border border-gray-300 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:outline-none">
                </div>
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="問合せの詳細を記入">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
