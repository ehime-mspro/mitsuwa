...
<div x-data="inquiryEditForm()">
    ...
    <form method="POST" action="{{ route('tenant.inquiries.update', $inquiry) }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="text" inputmode="decimal" pattern="[0-9.]*" name="desired_area_min" value="{{ old('desired_area_min', $inquiry->desired_area_min) }}"
                               class="form-input w-full h-[40px] px-3 border border-gray-300 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:outline-none"
                               placeholder="下限">
                        <span class="text-gray-400 text-sm flex-shrink-0">〜</span>
                        <input type="text" inputmode="decimal" pattern="[0-9.]*" name="desired_area_max" value="{{ old('desired_area_max', $inquiry->desired_area_max) }}"
                               class="form-input w-full h-[40px] px-3 border border-gray-300 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:outline-none"
                               placeholder="上限">
                    </div>
                </div>
        ...
        <x-form-actions submit-label="更新する" :cancel-url="route('tenant.inquiries.show', $inquiry)" />
    </form>
</div>
@endsection
