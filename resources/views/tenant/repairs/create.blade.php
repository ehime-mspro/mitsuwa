...
<div x-data="{
    ...
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900 mb-4">一般修繕 新規登録</h1>

    {{-- 判断ガイド --}}
    <div class="mb-4 rounded-lg border border-amber-300 bg-amber-50 p-4">
        <div class="text-sm font-bold text-amber-800 mb-2">💡 登録先の判断ガイド</div>
        <p class="text-sm text-amber-900 mb-1">
            店舗の新装・改修・解体新装など、テナントへの貸し出しを目的とした工事は → <strong>【投資案件】</strong>から登録してください
        </p>
        <p class="text-sm text-amber-900 mb-2">
            エアコン交換・給排水修理など通常のメンテナンスは → こちら<strong>【一般修繕】</strong>から登録してください
        </p>
        <a href="{{ route('tenant.investments.create') }}"
           class="inline-flex items-center gap-1 text-sm font-semibold text-emerald-600 hover:underline">
            投資案件を登録する →
        </a>
    </div>
    ...
    <form method="POST" action="{{ route('tenant.repairs.store') }}" enctype="multipart/form-data">
        @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <select name="property_id" x-model="propertyId"
                        ...
                        <optgroup label="稼働中">
                            @foreach($activeProps as $prop)
                                <option value="{{ $prop->id }}" {{ old('property_id') == $prop->id ? 'selected' : '' }}>{{ $prop->name }}（{{ $prop->code }}）</option>
                            @endforeach
                        </optgroup>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(\App\Enums\RepairStatus::cases() as $s)
                            <option value="{{ $s->value }}" {{ old('status', 'planned') === $s->value ? 'selected' : '' }}>{{ $s->label() }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="修繕の詳細を記入">{{ old('description') }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="cost" value="{{ old('cost') }}" min="0"
        ...
        <x-form-actions submit-label="登録する" :cancel-url="route('tenant.repairs.index')" />
    </form>
</div>
@endsection
