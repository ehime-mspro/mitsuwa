...
@section('breadcrumb')
    ...
    <span class="mx-1.5">›</span>
    <span class="text-gray-600">区画 新規登録</span>
@endsection
...
@section('content')
    ...
    {{-- ページタイトル --}}
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900 mb-4">区画 新規登録</h1>
    ...
    <form method="POST" action="{{ route('tenant.units.store', $property) }}">
        @csrf
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" inputmode="decimal" pattern="[0-9.]*" name="area_tsubo" value="{{ old('area_tsubo') }}"
                               class="form-input w-full h-[40px] px-3 pr-8 border border-gray-300 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 focus:outline-none"
                               placeholder="例: 15.30">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-4 py-1.5">
                        <label class="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                            <input type="radio" name="status" value="vacant" {{ old('status', 'vacant') === 'vacant' ? 'checked' : '' }}
                                   class="w-4 h-4 accent-emerald-600 cursor-pointer">
                            空室
                        </label>
                        <label class="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                            <input type="radio" name="status" value="negotiating" {{ old('status') === 'negotiating' ? 'checked' : '' }}
                                   class="w-4 h-4 accent-emerald-600 cursor-pointer">
                            商談中
                        </label>
                    </div>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-2">
            <x-form-actions submit-label="登録する" :cancel-url="route('tenant.properties.show', $property)" />
        </div>
    </form>
