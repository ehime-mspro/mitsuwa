...
<div x-data="{ showDeleteModal: false }">
    ...
    {{-- 成功メッセージ --}}
    @if(session('success'))
        <div class="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 font-medium">
            {{ session('success') }}
        </div>
    @endif
        ...
        <div class="bg-rose-50 border border-rose-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                ...
                <div>
                    ...
                    <div class="text-sm font-semibold text-gray-900">{{ $investment->recovery_start_date?->format('Y/m/d') ?? '—' }}</div>
                </div>
