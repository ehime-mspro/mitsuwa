...
@section('title', $property->name . ' — 編集')
...
@section('content')
<div x-data="{ ownerType: '{{ old('owner_type', $property->owner_type?->value ?? 'self_owned') }}' }">
    ...
    {{-- ページタイトル --}}
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900 mb-4">{{ $property->name }} — 編集</h1>
    ...
    <form method="POST" action="{{ route('tenant.properties.update', $property) }}">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">物件コード</label>
                    <input type="text" value="{{ $property->code }}" readonly
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-4 py-1.5">
                        <label class="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                            <input type="radio" name="operation_status" value="active"
                                   {{ old('operation_status', $property->operation_status->value) === 'active' ? 'checked' : '' }}
                                   class="w-4 h-4 accent-emerald-600 cursor-pointer">
                            稼働（入居募集中）
                        </label>
                        <label class="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                            <input type="radio" name="operation_status" value="inactive"
                                   {{ old('operation_status', $property->operation_status->value) === 'inactive' ? 'checked' : '' }}
                                   class="w-4 h-4 accent-emerald-600 cursor-pointer">
                            非稼働（募集停止）
                        </label>
                    </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <input type="month" name="built_date" value="{{ old('built_date', $property->built_date) }}"
                           class="form-input w-full h-[40px] px-3 border border-gray-300 rounded-md text-sm text-gray-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 focus:outline-none">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">総階数</label>
                    <div class="relative">
                        <input type="number" name="total_floors" value="{{ old('total_floors', $property->total_floors) }}" min="1" max="99"
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-3">
            ...
            <textarea name="notes" rows="3"
                      ...
                      placeholder="メモや補足情報があれば入力">{{ old('notes', $property->notes) }}</textarea>
        </div>

        {{-- 変更履歴の案内 --}}
        <div class="bg-amber-50 border border-amber-200 rounded-md px-3.5 py-2.5 mb-3 text-sm text-amber-800">
            <strong>変更履歴について:</strong> 保存時、変更された項目は自動的に変更履歴に記録されます。
        </div>
