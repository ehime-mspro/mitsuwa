    ...
    <table class="w-full border-collapse" style="table-layout: fixed;">
        ...
        <thead>
            <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 bg-gray-50 border-b border-gray-200 whitespace-nowrap">発注合計額</th>
...
@if($subcontractors->hasPages())
    <div class="mt-4 flex justify-center gap-0.5">
        ...
        @endif
        @foreach($subcontractors->getUrlRange(1, $subcontractors->lastPage()) as $page => $url)
            @if($page == $subcontractors->currentPage())
            ...
            @endif
        @endforeach
        @if($subcontractors->hasMorePages())
            <a href="{{ $subcontractors->nextPageUrl() }}"
               class="w-8 h-8 flex items-center justify-center rounded text-xs text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 transition-colors">&gt;</a>
        @else
            <span class="w-8 h-8 flex items-center justify-center rounded text-xs text-gray-300 bg-white border border-gray-200">&gt;</span>
        @endif
