{{-- DAD 工事案件 動的行用 datePicker パーシャル（x-for 内で使用） --}}
{{-- 期待される変数（すべて JS 式の文字列）:
     - $valueExpr  : 初期値の JS 式（例: "a.start_date"）
     - $nameExpr   : input[name] の JS 式（例: "'assignments[' + idx + '][start_date]'"）
     - $assignExpr : selected → 同期先の JS 式（例: "a.start_date"）
--}}
<div class="date-picker-wrap"
     x-data="datePicker({{ $valueExpr }})"
     x-init="$watch('selected', function (val) {
         {{ $assignExpr }} = val
    ...
    <input type="hidden" :name="{{ $nameExpr }}" :value="isoValue">
    ...
    <div class="picker-popup" x-show="open" x-transition style="display:none;">
        ...
        <div class="cal-grid" x-show="mode === 'calendar'">
            ...
            <template x-for="(cell, cidx) in calendarCells" :key="cidx">
