...
@section('title', '発注者 編集 - ' . $client->name)
...
<div class="flex items-center justify-between mb-4">
    <h1 class="text-lg max-lg:text-base font-bold text-gray-900">発注者 編集 — {{ $client->name }}</h1>
...
<div x-data="{ confirmDelete: false }" style="max-width: 1000px;">
    ...
    <form method="POST" action="{{ route('dad.clients.update', $client) }}">
        @csrf
        @method('PUT')
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="display: flex; gap: 8px;">
                <a href="{{ route('dad.clients.index') }}" style="display: inline-flex; align-items: center; padding: 10px 20px; border: 1px solid #d1d5db; border-radius: 6px; background: white; font-size: 14px; color: #374151; text-decoration: none;">キャンセル</a>
    ...
    <div x-show="confirmDelete" x-cloak class="mt-4 rounded-lg border border-red-200 bg-red-50 p-4">
        ...
        <p class="text-xs text-red-600 mb-3">※ 工事案件で使用中の場合、削除できません。</p>
